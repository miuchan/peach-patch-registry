...
const exportedFunctions='["_rack_web_param_count","_rack_web_input_count","_rack_web_output_count","_rack_web_light_count","_rack_web_max_channels","_rack_web_input_buffer","_rack_web_output_buffer","_rack_web_light_buffer","_rack_web_set_param","_rack_web_reset_param","_rack_web_get_param","_rack_web_get_param_min","_rack_web_get_param_max","_rack_web_set_input_connected","_rack_web_set_output_connected","_rack_web_set_input_channels","_rack_web_get_output_channels","_rack_web_set_polyphony","_rack_web_set_state","_rack_web_state_buffer","_rack_web_commit_state_json","_rack_web_snapshot_state_json","_rack_web_snapshot_state_buffer","_rack_web_trigger_action","_rack_web_asset_capacity","_rack_web_asset_buffer","_rack_web_commit_asset","_rack_web_capture_capacity","_rack_web_capture_buffer","_rack_web_capture_frames","_rack_web_capture_channels","_rack_web_capture_active","_rack_web_consume_capture","_rack_web_set_capture_enabled","_rack_web_expander_capacity","_rack_web_expander_input_buffer","_rack_web_expander_output_buffer","_rack_web_set_expander_count","_rack_web_set_expander_type","_rack_web_set_expander_bypassed","_rack_web_set_expander_param","_rack_web_set_expander_input_connected","_rack_web_set_expander_input_channels","_rack_web_get_expander_output_channels","_rack_web_message_capacity","_rack_web_set_message_neighbor","_rack_web_set_message_chain_neighbor","_rack_web_set_chain_neighbor_bypassed","_rack_web_set_chain_neighbor_param","_rack_web_get_chain_neighbor_param","_rack_web_set_chain_neighbor_input","_rack_web_get_chain_neighbor_input_channels","_rack_web_get_chain_neighbor_input_voltage","_rack_web_set_chain_neighbor_output_connected","_rack_web_get_chain_neighbor_output_channels","_rack_web_get_chain_neighbor_output_voltage","_rack_web_set_neighbor_bypassed","_rack_web_set_neighbor_param","_rack_web_set_neighbor_input","_rack_web_set_neighbor_output_connected","_rack_web_get_neighbor_output_channels","_rack_web_get_neighbor_output_voltage","_rack_web_message_buffer","_rack_web_message_flip_requested","_rack_web_finish_message_flip","_rack_web_seed","_rack_web_process_frame","_rack_web_process"]';

const linkedExportedFunctions=exportedFunctions
  .replace('"_rack_web_light_buffer"','"_rack_web_light_buffer","_rack_web_visual_count","_rack_web_visual_buffer"')
  .replace('"_rack_web_trigger_action"','"_rack_web_trigger_action","_rack_web_midi_push","_rack_web_midi_output_available","_rack_web_midi_output_buffer","_rack_web_consume_midi_output","_rack_web_midi_packet_output_available","_rack_web_midi_packet_output_buffer","_rack_web_consume_midi_packet_output"')
  .replace('"_rack_web_commit_asset"','"_rack_web_commit_asset","_rack_web_asset_slot_count","_rack_web_asset_capacity_for_slot","_rack_web_asset_buffer_for_slot","_rack_web_commit_asset_for_slot"');
...
function referencedSiblingModuleDefinitions(sourceFiles,targetBody,excludedNames=new Set){
  ...
  for(const {source} of sources)for(const match of source.matchAll(/\b(?:struct|class)\s+([A-Za-z_]\w*)\s*:\s*([^\{]+)\{/g)){
    const name=match[1],reference=new RegExp(`(?:\\b${name}\\s*\\*|\\b${name}::)`);
    if(excludedNames.has(name)||/(?:Widget|Display)$/.test(name)||rackUiPattern.test(match[0])||!reference.test(targetBody))continue;
    const body=structBody(source,name),base=declaredBases(source,name)[0],baseName=baseTypeName(base);
    if(body===null||!base||(!rackModuleBase(base)&&!/Module|Expander/i.test(baseName)))continue;
    const implementations=sources.flatMap(candidate=>outOfLineDefinitions(candidate.source,name)).filter(value=>!/\b(?:draw|drawLayer)\s*\([^;{}]*\b(?:widget::Widget::)?DrawArgs\b/.test(value));
...
function withCacheLock(lockPath,callback){
  fs.mkdirSync(path.dirname(lockPath),{recursive:true});const started=Date.now(),waitBuffer=new Int32Array(new SharedArrayBuffer(4));
  while(true)try{fs.mkdirSync(lockPath);break}catch(error){if(error?.code!=="EEXIST")throw error;try{if(Date.now()-fs.statSync(lockPath).mtimeMs>120000)fs.rmdirSync(lockPath)}catch{}if(Date.now()-started>30000)fail(`Timed out waiting for source cache lock ${path.basename(lockPath)}`);Atomics.wait(waitBuffer,0,0,50)}
  try{return callback()}finally{try{fs.rmdirSync(lockPath)}catch{}}
}
function refreshLibraryIndex(indexDir){
  return withCacheLock(`${indexDir}.lock`,()=>{const gitDir=path.join(indexDir,".git");if(!fs.existsSync(gitDir)){if(fs.existsSync(indexDir))fail(`Library index cache is incomplete at ${indexDir}`);const staging=`${indexDir}.building-${process.pid}`;execFileSync("git",["clone","--depth","1","--filter=blob:none","--branch","v2","--no-checkout","https://github.com/VCVRack/library.git",staging],{stdio:"inherit"});fs.renameSync(staging,indexDir);return}const fetchHead=path.join(gitDir,"FETCH_HEAD"),fresh=fs.existsSync(fetchHead)&&Date.now()-fs.statSync(fetchHead).mtimeMs<30000;if(fresh)return;try{execFileSync("git",["-C",indexDir,"fetch","--depth","1","origin","v2"],{stdio:"inherit"})}catch{if(!fs.existsSync(path.join(gitDir,"HEAD")))throw new Error("Could not refresh or reuse the VCV Library index")}})
}
export function officialLibrarySubmodule(modules,plugin,sourceUrl,allowCommitLockedMirror=false){
  const repository=sourceRepository(sourceUrl),entries=[...modules.matchAll(/\[submodule\s+"([^"]+)"\]([\s\S]*?)(?=\n\[submodule\s+"|$)/g)].map(match=>{const body=match[2],pathValue=/^\s*path\s*=\s*(\S+)\s*$/m.exec(body)?.[1],url=/^\s*url\s*=\s*(\S+)\s*$/m.exec(body)?.[1];return{name:match[1],path:pathValue,url}}).filter(entry=>entry.path&&entry.url),byPath=entries.find(entry=>entry.path===`repos/${plugin}`),byRepository=entries.filter(entry=>{try{return sourceRepository(entry.url)===repository}catch{return false}}),selected=byPath??(byRepository.length===1?byRepository[0]:undefined);
  if(!selected)return undefined;
  if(!/^repos\/[A-Za-z0-9_.-]+$/.test(selected.path))fail("Official Library submodule path is not a safe repository entry");
  if(sourceRepository(selected.url)!==repository&&!allowCommitLockedMirror)fail("Official manifest sourceUrl differs from its Library submodule");
  return selected;
}
export function sourceRevisionFromRemote(repository,version){const tags=[`v${version}`,String(version),`vcv-v${version}`],refs=execFileSync("git",["ls-remote",repository,...tags.flatMap(tag=>[`refs/tags/${tag}`,`refs/tags/${tag}^{}`])],{encoding:"utf8"}).trim().split("\n").filter(Boolean),tagCommit=(refs.find(line=>line.endsWith("^{}"))??refs[0]??"").split(/\s+/)[0];if(/^[0-9a-f]{40}$/.test(tagCommit))return tagCommit;const head=execFileSync("git",["ls-remote",repository,"HEAD"],{encoding:"utf8"}).trim().split(/\s+/)[0];return /^[0-9a-f]{40}$/.test(head)?head:""}
const legacyLibrarySourceLocks=new Map([
  ["RJModules@1.7.2",{repository:"https://github.com/Miserlou/RJModules.git",commit:"7db2aadc1c2521365bf200a3c42ed0f90bbd841a"}],
  ["TheXOR@1.1.1",{repository:"https://github.com/spectromas/RackPlugins.git",commit:"168a32e1331f7eb355b1522ae0550186bebba45a"}],
  ["luckyxxl@1.0.0",{repository:"https://github.com/eriser/vcv_luckyxxl.git",commit:"13b76875e980503c1d123edded34a310cb2588a6"}],
  ["RacketScience@1.1.0",{repository:"https://github.com/ContemporaryInsanity/RacketScience.git",commit:"04caab44e47d60b3af2a9fae0108e9cc241f31ac"}],
]);
export function legacyLibrarySourceLock(plugin,version){return legacyLibrarySourceLocks.get(`${plugin}@${version}`)}
function officialSourceLock(plugin,sourceUrl,version,indexDir){refreshLibraryIndex(indexDir);const revision=gitValue(indexDir,["rev-parse","--verify","FETCH_HEAD^{commit}"])?"FETCH_HEAD":"HEAD",modules=execFileSync("git",["-C",indexDir,"show",`${revision}:.gitmodules`],{encoding:"utf8"}),submodule=officialLibrarySubmodule(modules,plugin,sourceUrl,true),legacy=!submodule&&legacyLibrarySourceLock(plugin,version),repository=submodule?sourceRepository(submodule.url):legacy?.repository??sourceRepository(sourceUrl),commit=submodule?execFileSync("git",["-C",indexDir,"rev-parse",`${revision}:${submodule.path}`],{encoding:"utf8"}).trim():legacy?.commit??sourceRevisionFromRemote(repository,version);if(!/^[0-9a-f]{40}$/.test(commit))fail(`Could not resolve an exact source revision for ${plugin} ${version}`);return{commit,repository}}
async function officialManifest(plugin,indexDir){try{const response=await fetch(`https://raw.githubusercontent.com/VCVRack/library/v2/manifests/${plugin}.json`);if(!response.ok)fail(`Official Library manifest returned ${response.status}`);return await response.json()}catch(error){if(fs.existsSync(path.join(indexDir,".git"))){for(const revision of ["FETCH_HEAD","HEAD"]){try{return JSON.parse(execFileSync("git",["-C",indexDir,"show",`${revision}:manifests/${plugin}.json`],{encoding:"utf8"}))}catch{}}}throw error}}
...
export function checkoutLockedRepository(repository,commit,target){
...
function checkoutSource(repository,commit,target){const staging=checkoutLockedRepository(repository,commit,target);fs.renameSync(staging,target)}
function cachedSourceCheckout(cacheRoot,target){if(!cacheRoot)return null;const pluginRoot=path.join(path.resolve(cacheRoot),target.plugin);if(!fs.existsSync(pluginRoot))return null;const candidates=[];for(const entry of fs.readdirSync(pluginRoot,{withFileTypes:true})){if(!entry.isDirectory()||!/^[0-9a-f]{40}$/.test(entry.name))continue;const directory=path.join(pluginRoot,entry.name),manifestPath=path.join(directory,"plugin.json");if(!fs.existsSync(manifestPath)||!fs.existsSync(path.join(directory,".git")))continue;try{const manifest=JSON.parse(fs.readFileSync(manifestPath,"utf8")),head=execFileSync("git",["-C",directory,"rev-parse","HEAD"],{encoding:"utf8",stdio:["ignore","pipe","ignore"]}).trim();if(head!==entry.name||manifest.slug!==target.plugin||!manifest.modules?.some(module=>module.slug===target.model)||!manifest.sourceUrl)continue;candidates.push({directory,commit:head,manifest})}catch{}}return candidates.sort((left,right)=>String(right.manifest.version??"").localeCompare(String(left.manifest.version??""),undefined,{numeric:true}))[0]??null}
...
function hydrateLockedFallbackDependencies(sourceDir){
  const rootFiles=filesOutsideNestedRepositories(sourceDir),referencedIncludes=new Set(rootFiles.flatMap(file=>includes(fs.readFileSync(file,"utf8")))),availableFiles=files(sourceDir);
  for(const dependency of lockedFallbackDependencies){
    ...
    checkoutSource(dependency.repository,dependency.commit,target);
  }
}
function hydrateWrongPeopleLuaRuntime(sourceDir,target){
  ...
  if(!fs.existsSync(path.join(root,".git"))){
    ...
    const staging=checkoutLockedRepository("https://github.com/lua/lua.git",commit,root);
...
function lockedFallbackDependencyBundleForAdapter(sourceFiles,source){
  const referenced=new Set(includes(source)),bundles=[];
  for(const dependency of lockedFallbackDependencies){
...
function files(root){const found=[];for(const entry of fs.readdirSync(root,{withFileTypes:true})){if([".git","build","dist","dep","deps","node_modules","third_party","CMSIS","test","tests","examples"].includes(entry.name)||entry.name.includes(".building-")||/^STM32.*StdPeriph_Driver$/.test(entry.name))continue;const target=path.join(root,entry.name);if(entry.isDirectory())found.push(...files(target));else if(/\.(?:c|cpp|cc|cxx|hpp|hh|h|inl)$/.test(entry.name))found.push(target)}return found}
...
export function widgetPanelWidth(sourceRoot,registeredWidgetClass){
  ...
  while(queue.length){
    ...
    for(const file of sourceFiles){
      const source=fs.readFileSync(file,"utf8"),body=plainStructBody(source,name);if(body===null)continue;
      const templateDeclaration=new RegExp(`\\btemplate\\s*<([^>]+)>\\s*(?:struct|class)\\s+${name}\\b`).exec(source),constants={...current.constants};
      if(templateDeclaration&&typeArguments){
        const parameters=splitArguments(templateDeclaration[1]).map(value=>/([A-Za-z_]\w*)\s*(?:=[^,]+)?$/.exec(value)?.[1]),values=splitArguments(typeArguments);
      ...
      const widgetSource=[body,...sourceFiles.flatMap(candidate=>outOfLineDefinitions(fs.readFileSync(candidate,"utf8"),name))].join("\n"),hpExpressions=[...widgetSource.matchAll(/\bhp\s*=\s*([^;\n]+)\s*;/g)].map(match=>match[1]);
      ...
      for(const base of declaredBases(source,name))queue.push({type:base,constants});
    }
  }
  return null;
}
...
function filesOutsideNestedRepositories(root,current=root){
  if(current!==root&&fs.existsSync(path.join(current,".git")))return[];
  const found=[],developmentDirectories=new Set([".git","bench","benchmark","build","cmdline","dist","doc","docs","dep","deps","examples","node_modules","test","tests","unittest"]);
  for(const entry of fs.readdirSync(current,{withFileTypes:true})){
    if(developmentDirectories.has(entry.name))continue;
    const target=path.join(current,entry.name);
    if(entry.isDirectory())found.push(...filesOutsideNestedRepositories(root,target));
    else if(/\.(?:c|cpp|cc|cxx|hpp|hh|h|inl)$/.test(entry.name)&&entry.name!=="miniaudio.h")found.push(target);
  }
  return found
}
function repositoryRoots(root){
  ...
  function visit(current){
    const modulesFile=path.join(current,".gitmodules");
    if(!fs.existsSync(modulesFile))return;
    let declarations="";
    try{declarations=execFileSync("git",["config","-f",modulesFile,"--get-regexp","^submodule\\..*\\.path$"],{encoding:"utf8"}).trim()}
    catch(error){if(error?.status===1)return;throw error}
    for(const line of declarations.split("\n").filter(Boolean)){
      const relative=/^submodule\..+\.path\s+(.+)$/.exec(line)?.[1];
      if(!relative)continue;
      const target=path.resolve(current,relative);
      // MetaModule's SDK is a cross-compilation toolchain, not plugin DSP.
      // Treating its newlib and firmware sources as Rack dependencies causes
      // the browser linker to compile a second, incompatible C runtime.
      if(path.basename(target)==="metamodule-plugin-sdk")continue;
      if(fs.existsSync(path.join(target,".git"))){found.push(target);visit(target)}
    }
  }
  visit(root);
  return found
...
function initializedGlobalNames(source){return new Set([...String(source).matchAll(/^(?:const|constexpr|static)\s+(?:[A-Za-z_]\w*(?:::\w+)*(?:\s*[*&])?\s+)+([A-Za-z_]\w*)\s*(?:\[[^\]]*\])?\s*=/gm)].map(match=>match[1]))}
function duplicatesHeaderGlobals(headerSource,implementationFile){const names=initializedGlobalNames(headerSource);if(!names.size)return false;const defined=initializedGlobalNames(fs.readFileSync(implementationFile,"utf8"));return[...names].some(name=>defined.has(name))}
function pruneInactiveConditionalDependencies(found,sourceDir){
  const headers=found.filter(file=>/\.(?:hpp|hh|h|inl)$/.test(file)),conditionNames=new Set(headers.flatMap(file=>[...fs.readFileSync(file,"utf8").matchAll(/^\s*#\s*(?:if|elif)\s*!?\s*([A-Za-z_]\w*)\s*$/gm)].map(match=>match[1]))),values=new Map;
  for(const file of headers)for(const [name,value] of objectMacroDefinitions(fs.readFileSync(file,"utf8"))){if(!conditionNames.has(name))continue;if(!values.has(name))values.set(name,new Set);values.get(name).add(value)}
  ...
  for(const file of headers){
    const raw=fs.readFileSync(file,"utf8"),active=preprocessMacroSource(raw,definitions).source,rawTargets=new Set(includes(raw).map(include=>resolve(file,include)).filter(Boolean)),activeTargetsForFile=new Set(includes(active).map(include=>resolve(file,include)).filter(Boolean));
...
function includedDependencyFiles(sourceDir,roots){
  const repositories=repositoryRoots(sourceDir),sourceRoot=path.resolve(sourceDir),sourcePrefix=`${sourceRoot}${path.sep}`,firmwareLibc=file=>/(?:^|[\\/])(?:metamodule-)?plugin-libc(?:[\\/]|$)/.test(path.relative(sourceRoot,file)),indexedFiles=files(sourceRoot).filter(file=>!firmwareLibc(file)),suffixes=new Map;
  for(const file of indexedFiles){const basename=path.basename(file);if(!suffixes.has(basename))suffixes.set(basename,[]);suffixes.get(basename).push(file)}
  const found=new Set,companionImplementations=new Set,queue=[...roots];while(queue.length){const file=queue.shift();if(!file||found.has(file)||!fs.existsSync(file))continue;found.add(file);const source=fs.readFileSync(file,"utf8"),includeDirectives=[...source.matchAll(/^\s*#include\s*([<"])([^>"]+)[>"]/gm)].map(match=>({angle:match[1]==="<",include:match[2]}));for(const {angle,include} of includeDirectives){
    if(path.basename(include)==="rack.hpp")continue;
    if(angle&&!include.includes("/")&&!/[A-Z]/.test(include)&&!/^lib[A-Z]/.test(include))continue;
    const direct=[path.resolve(path.dirname(file),include),...repositories.map(root=>path.resolve(root,include)),path.resolve(sourceRoot,"src",include)].find(candidate=>(candidate===sourceRoot||candidate.startsWith(sourcePrefix))&&!firmwareLibc(candidate)&&fs.existsSync(candidate)&&fs.statSync(candidate).isFile());
    const normalized=include.split(/[\\/]+/).join(path.sep),suffix=`${path.sep}${normalized}`,matches=direct?[]:(suffixes.get(path.basename(normalized))??[]).filter(candidate=>candidate.endsWith(suffix)),importerRepository=[...repositories].filter(root=>file===root||file.startsWith(`${root}${path.sep}`)).sort((left,right)=>right.length-left.length)[0],repositoryMatches=importerRepository?matches.filter(candidate=>candidate.startsWith(`${importerRepository}${path.sep}`)):[],dependency=direct??selectIncludeCandidate(repositoryMatches.length?repositoryMatches:matches,sourceDir);
    if(dependency&&!found.has(dependency))queue.push(dependency)
  }if(/\.(?:hpp|hh|h|inl)$/.test(file)&&path.basename(file)!=="plugin.hpp"){const stem=file.replace(/\.(?:hpp|hh|h|inl)$/,"");for(const extension of [".c",".cpp",".cc",".cxx"]){const localImplementation=`${stem}${extension}`,sameStem=indexedFiles.filter(candidate=>candidate.endsWith(extension)&&path.basename(candidate,extension)===path.basename(stem)&&!path.relative(sourceRoot,candidate).split(path.sep).some(part=>["sample","samples","test","tests","example","examples"].includes(part))),implementation=fs.existsSync(localImplementation)?localImplementation:(sameStem.length===1?sameStem[0]:undefined);if(implementation&&!duplicatesHeaderGlobals(source,implementation)&&!found.has(implementation)){companionImplementations.add(implementation);queue.push(implementation)}}}}return pruneInactiveConditionalDependencies([...found],sourceDir)
}
...
function standardDependencyIncludes(source){const active=preprocessMacroSource(source,new Map,false).source;return[...active.matchAll(/^\s*#include\s+<([A-Za-z0-9_./-]+)>/gm)].map(match=>match[1]).filter(browserSafeSystemInclude).map(include=>`#include <${include}>`)}
function externalDependencyPrelude(sourceDir,source,sourceFiles=[]){const externalRoots=repositoryRoots(sourceDir).slice(1),resolved=standardDependencyIncludes(source),quoted=[...source.matchAll(/^\s*#include\s+"([^"]+)"/gm)].map(match=>match[1]);for(const include of quoted){if(path.basename(include)==="plugin.hpp")continue;const normalized=include.split(/[\\/]+/).join(path.sep),direct=externalRoots.some(root=>fs.existsSync(path.resolve(root,normalized))),matches=sourceFiles.filter(file=>externalRoots.some(root=>file.startsWith(`${root}${path.sep}`))&&file.endsWith(`${path.sep}${normalized}`));if(direct||selectIncludeCandidate(matches,sourceDir))resolved.push(`#include "${include}"`)}return[...new Set(resolved)].join("\n")}
function vendoredDependencyBundleForAdapter(sourceDir,definitionFile,sourceFiles){
  ...
  for(const include of includes(source)){const normalized=include.split(/[\\/]+/).join(path.sep),parts=normalized.split(path.sep),index=parts.findIndex(part=>vendorNames.has(part));if(index>=0){containers.add(path.resolve(path.dirname(definitionFile),...parts.slice(0,index+1)));containers.add(path.resolve(sourceDir,"src",...parts.slice(0,index+1)));containers.add(path.resolve(sourceDir,...parts.slice(0,index+1)))}}
  const vendorFiles=[];function visit(directory){if(!fs.existsSync(directory))return;for(const entry of fs.readdirSync(directory,{withFileTypes:true})){if(entry.name===".git"||["build","dist","node_modules"].includes(entry.name))continue;const target=path.join(directory,entry.name);if(entry.isDirectory())visit(target);else if(/\.(?:c|cpp|cc|cxx|hpp|hh|h)$/.test(entry.name))vendorFiles.push(target)}}for(const container of containers)visit(container);
...
function uiHeaderRequired(file,sourceDir,seen=new Set){if(seen.has(file))return false;seen.add(file);const source=fs.readFileSync(file,"utf8"),code=sourceWithoutComments(source),nativeRackUi=/\b(?:ModuleWidget|ParamWidget|PortWidget|LightWidget|OpaqueWidget|TransparentWidget|FramebufferWidget|SvgWidget|SvgKnob|SvgPanel|PanelBorder|DrawArgs|NVGcontext|MenuItem|MenuSeparator)\b|\b(?:app|ui|widget|window)::|\b(?:createParam|createInput|createOutput|createMenu|nvg[A-Z]\w*)\s*\(/.test(code);if(nativeRackUi)return true;return includes(source).some(include=>{const target=[path.resolve(path.dirname(file),include),path.resolve(sourceDir,"src",include),path.resolve(sourceDir,include)].find(candidate=>fs.existsSync(candidate)&&fs.statSync(candidate).isFile());return target?uiHeaderRequired(target,sourceDir,seen):false})}
function stripUiHeaderIncludes(source,sourceFiles=[],sourceDir=""){return source.replace(/^\s*#include\s+"([^"]+)"\s*$/gm,(line,include)=>{const normalized=include.split("/").join(path.sep),target=sourceFiles.find(file=>file.endsWith(`${path.sep}${normalized}`))||[path.resolve(sourceDir,"src",normalized),path.resolve(sourceDir,normalized)].find(file=>fs.existsSync(file)&&fs.statSync(file).isFile());if(!target)return line;return uiHeaderRequired(target,sourceDir)?"":line}).replace(/^\s*namespace\s+[A-Za-z_]\w*\s*=\s*ghc::filesystem\s*;\s*$/gm,"")}
function stripPluginInitFunctions(source){let result=source;for(let pass=0;pass<16;pass++){const match=/\bvoid\s+init\s*\(\s*(?:rack::)?Plugin\s*\*\s*[A-Za-z_]\w*\s*\)\s*\{/.exec(result);if(!match)break;const open=result.indexOf("{",match.index),close=matchingBrace(result,open);if(close<0)break;result=result.slice(0,match.index)+result.slice(close+1)}return result.replace(/^\s*(?:(?:rack::)?plugin::)?Model\s*\*\s*[A-Za-z_]\w*\s*=\s*create[A-Za-z0-9_]*Model\s*<[^;]+>\s*\([^;]*\)\s*;\s*$/gm,"")}
function stripOrphanPreprocessorEnds(source){
  let depth=0;
  return source.split("\n").filter(line=>{
    if(/^\s*#\s*(?:if|ifdef|ifndef)\b/.test(line)){depth++;return true}
    if(!/^\s*#\s*endif\b/.test(line))return true;
...
function stripHeaderGuardOpen(source){
  ...
  const guard=/^\s*#(?:ifndef\s+([A-Za-z_]\w*)|if\s+!\s*defined(?:\s*\(\s*([A-Za-z_]\w*)\s*\)|\s+([A-Za-z_]\w*)))[^\n]*\n\s*#define[ \t]+([A-Za-z_]\w*)(?:[ \t]+[^\r\n]+)?[ \t]*\r?$/m.exec(source);
  if(!guard||(guard[1]??guard[2]??guard[3])!==guard[4])return source;
  const withoutOpen=source.slice(0,guard.index)+source.slice(guard.index+guard[0].length);
  return stripOrphanPreprocessorEnds(withoutOpen);
}
...
function typeAlias(sourceFiles,type){if(String(type).includes("<"))return null;const name=baseTypeName(type),escaped=name.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");for(const file of sourceFiles){const source=fs.readFileSync(file,"utf8"),typedef=new RegExp("\\btypedef\\s+([^;]+?)\\s+"+escaped+"\\s*;").exec(source)?.[1],using=new RegExp("\\busing\\s+"+escaped+"\\s*=\\s*([^;]+);").exec(source)?.[1],resolved=(typedef??using)?.trim();if(resolved&&resolved!==name)return resolved}return null}
...
function declaredBases(source,className){const name=baseTypeName(className),match=new RegExp(`\\b(?:struct|class)\\s+${name}(?:\\s+final)?\\s*:\\s*([^\\{]+)\\{`).exec(source);if(!match)return[];return splitArguments(match[1]).map(value=>value.replace(/\b(?:public|protected|private|virtual)\b/g,"").trim()).filter(Boolean)}
function rackModuleBase(type){return /^(?:(?:rack::)?(?:engine::)?)?Module$/.test(String(type).replace(/\s/g,""))}
function qualifyScopedAlias(type,namespaces){const value=String(type).replace(/^typename\s+/,"").trim();if(value.startsWith("::"))return value.slice(2);const parts=value.split("::"),shared=namespaces.lastIndexOf(parts[0]);if(shared>=0)return[...namespaces.slice(0,shared+1),...parts.slice(1)].join("::");if(parts[0]===namespaces[0]||["rack","std"].includes(parts[0]))return value;const prefix=/ui$/i.test(namespaces.at(-1)??"")?namespaces.slice(0,-1):namespaces;return[...prefix,...parts].join("::")}
function resolveRegisteredModuleType(source,type){
  let resolved=String(type).trim();for(let depth=0;depth<8;depth++){
    if(/^[A-Za-z_]\w*$/.test(resolved)){
      const escaped=resolved.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),
        typedef=new RegExp(`\\btypedef\\s+([^;]+?)\\s+${escaped}\\s*;`).exec(source)?.[1],
        using=new RegExp(`\\busing\\s+${escaped}\\s*=\\s*([^;]+);`).exec(source)?.[1],
        value=(typedef??using)?.trim();
      if(value&&value!==resolved){resolved=value;continue}
    }
    const match=/^(.*)::([A-Za-z_]\w*)$/.exec(resolved);if(!match)break;const owner=match[1],alias=match[2],body=structBody(source,owner)??plainStructBody(source,owner);if(!body)break;const escaped=alias.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),typedef=new RegExp(`\\btypedef\\s+([^;]+?)\\s+${escaped}\\s*;`).exec(body)?.[1],using=new RegExp(`\\busing\\s+${escaped}\\s*=\\s*([^;]+);`).exec(body)?.[1];let value=(typedef??using)?.trim();if(!value)break;const contract=templateContract(source,owner);if(contract)value=substituteType(value,Object.fromEntries(contract.parameters.map((parameter,index)=>[parameter,contract.arguments[index]??parameter])));resolved=qualifyScopedAlias(value,enclosingNamespaces(source,owner))
  }return resolved
}
...
function registrationSourceContext(source,file){
  const chunks=[source],seen=new Set([path.resolve(file)]);
  const visit=(currentSource,currentFile)=>{
    if(chunks.length>=256)return;
    for(const include of [...currentSource.matchAll(/^\s*#\s*include\s+"([^"]+)"/gm)].map(match=>match[1])){
      const candidate=path.resolve(path.dirname(currentFile),include);
...
function modelRegistrations(source,file,stringConstants={}){
  const registrations=[];
  for(const start of source.matchAll(/\bcreate[A-Za-z0-9_]*Model\s*</g)){
    ...
    const slug=stringLiteral(splitArguments(source.slice(openParenthesis+1,closeParenthesis))[0],null,stringConstants);
    if(typeof slug!=="string"||!/^[A-Za-z0-9_-]+$/.test(slug))continue;
    const types=splitArguments(source.slice(depthStart+1,close));
    if(![1,2].includes(types.length)||types.some(type=>!/^[A-Za-z0-9_:<>,. +\-]+$/.test(type)))continue;
    const widgetClass=types.at(-1),registeredModuleClass=types.length===2?types[0]:registeredWidgetModuleType(source,file,widgetClass);
    if(!registeredModuleClass)continue;
    const registrationContext=types.length===1?registrationSourceContext(source,file):source;
    registrations.push({file,moduleClass:resolveRegisteredModuleType(registrationContext,registeredModuleClass),registeredModuleClass,widgetClass,slug,registrationNamespace:namespaceStackAt(source,start.index)})
  }
  for(const start of source.matchAll(/\b(?:plugin::)?Model\s*\*\s*model([A-Za-z0-9_-]+)\s*=\s*\[\s*\]\s*\(\s*\)\s*\{/g)){
...
function metaModuleRegistrations(source,file){
  const registrations=[];
  for(const start of source.matchAll(/\bGenericModule\s*</g)){
...
function registeredModuleDefinitionExists(sourceDir,registration){
  ...
  for(const include of includes(registrationSource))for(const candidate of [path.resolve(path.dirname(registration.file),include),path.resolve(root,"src",include),path.resolve(root,include)])if((candidate===root||candidate.startsWith(prefix))&&fs.existsSync(candidate)&&fs.statSync(candidate).isFile())candidates.push(candidate);
  return[...new Set(candidates)].some(file=>{const source=fs.readFileSync(file,"utf8");if(structBody(source,registration.moduleClass)===null)return false;const namespace=enclosingNamespaces(source,registration.moduleClass),expected=registration.registrationNamespace??[];return namespace.length===expected.length&&namespace.every((part,index)=>part===expected[index])});
}
function functionMacroRegistrationSource(source){
  const lines=source.split("\n"),definitions=[];
  for(let index=0;index<lines.length;index++){
    const declaration=/^[ \t]*#\s*define[ \t]+([A-Za-z_]\w*)[ \t]*\(([^)]*)\)[ \t]*(.*)$/.exec(lines[index]);if(!declaration)continue;
    let body=declaration[3];while(/\\\s*$/.test(body)&&index+1<lines.length)body=body.replace(/\\\s*$/," ")+lines[++index];
    if(!/\bcreateModel\s*</.test(body))continue;definitions.push({name:declaration[1],parameters:splitArguments(declaration[2]),body});
  }
  const expanded=[];
...
function objectMacroDefinitions(source){const definitions=new Map;for(const match of source.matchAll(/^[ \t]*#define[ \t]+([A-Za-z_]\w*)(?!\()(?:(?:[ \t]+)([^\n\\]*))?[ \t]*$/gm)){const value=String(match[2]??"").replace(/\s+\/\/.*$/," ").trim();definitions.set(match[1],value)}return definitions}
function macroDefinitionBlocks(source){const found=[],lines=source.split("\n");for(let index=0;index<lines.length;index++){const match=/^[ \t]*#\s*define[ \t]+([A-Za-z_]\w*)(?:[ \t]*\([^)]*\))?(?:[ \t]+.*)?$/.exec(lines[index]);if(!match)continue;const block=[lines[index]];while(/\\\s*$/.test(block.at(-1))&&index+1<lines.length)block.push(lines[++index]);found.push({name:match[1],definition:block.join("\n").trim()})}return found}
function replaceIdentifiersOutsideComments(source,definitions){let result="";for(let index=0;index<source.length;){const current=source[index],next=source[index+1];if(current==="/"&&next==="/"){const end=source.indexOf("\n",index);if(end<0)return result+source.slice(index);result+=source.slice(index,end+1);index=end+1;continue}if(current==="/"&&next==="*"){const end=source.indexOf("*/",index+2),limit=end<0?source.length:end+2;result+=source.slice(index,limit);index=limit;continue}if(current==='"'||current==="'"){const quote=current,start=index++;while(index<source.length){if(source[index]==="\\"){index+=2;continue}if(source[index++]===quote)break}result+=source.slice(start,index);continue}if(/[A-Za-z_]/.test(current)){let end=index+1;while(/[A-Za-z0-9_]/.test(source[end]??""))end++;const name=source.slice(index,end),value=definitions.get(name);result+=value||name;index=end;continue}result+=current;index++}return result}
function replaceObjectMacros(source,definitions){const protectedBlocks=[];let result=source.replace(/^[ \t]*#\s*define\b(?:[^\n]*\\\s*\n)*[^\n]*/gm,block=>{const token=`__RACK_WEB_MACRO_DEFINITION_${protectedBlocks.length}__`;protectedBlocks.push(block);return token});for(let pass=0;pass<16;pass++){const next=replaceIdentifiersOutsideComments(result,definitions);if(next===result)break;result=next}return result.replace(/__RACK_WEB_MACRO_DEFINITION_(\d+)__/g,(_,index)=>protectedBlocks[Number(index)]??"")}
function preprocessMacroSource(source,initialDefinitions,expandObjectMacros=true){
  source=source.replace(/\r\n?/g,"\n");
  const definitions=new Map(initialDefinitions),alwaysDefined=new Set(["TEST","VCV","NDEBUG","__EMSCRIPTEN__","__cplusplus"]),stack=[],output=[],lines=source.split("\n"),parentActive=()=>stack.every(frame=>frame.active);
  for(let lineIndex=0;lineIndex<lines.length;lineIndex++){
    const line=lines[lineIndex],directive=/^[ \t]*#\s*(ifdef|ifndef|if|elif|else|endif|define|undef)\b(?:[ \t]+(.*))?$/.exec(line);
    if(!directive){if(parentActive())output.push(line);continue}
    const command=directive[1],argument=String(directive[2]??"").trim(),defined=name=>definitions.has(name)||alwaysDefined.has(name),macroBoolean=name=>{if(alwaysDefined.has(name))return true;if(!definitions.has(name))return false;const value=String(definitions.get(name)??"").trim();if(!value)return true;if(/^[+-]?(?:0[xX][0-9a-fA-F]+|\d+)$/.test(value))return Number(value)!==0;return true},condition=()=>{if(command==="ifdef")return defined(argument);if(command==="ifndef")return!defined(argument);const positive=/^defined\s*\(\s*([A-Za-z_]\w*)\s*\)$/.exec(argument);if(positive)return defined(positive[1]);const negative=/^!\s*defined\s*\(\s*([A-Za-z_]\w*)\s*\)$/.exec(argument);if(negative)return!defined(negative[1]);const identifier=/^([A-Za-z_]\w*)$/.exec(argument);if(identifier)return macroBoolean(identifier[1]);const negated=/^!\s*([A-Za-z_]\w*)$/.exec(argument);if(negated)return!macroBoolean(negated[1]);return null};
    if(["ifdef","ifndef","if"].includes(command)){const value=condition();if(value===null){stack.push({managed:false,active:true,matched:false});if(parentActive())output.push(line)}else{const outer=parentActive();stack.push({managed:true,active:outer&&value,matched:Boolean(value),outer})}continue}
    if(command==="elif"){const frame=stack.at(-1);if(!frame?.managed){if(parentActive())output.push(line);continue}const value=condition();frame.active=Boolean(frame.outer&&!frame.matched&&value);frame.matched||=Boolean(value);continue}
    if(command==="else"){const frame=stack.at(-1);if(!frame?.managed){if(parentActive())output.push(line);continue}frame.active=Boolean(frame.outer&&!frame.matched);frame.matched=true;continue}
    if(command==="endif"){const frame=stack.pop();if(!frame?.managed&&parentActive())output.push(line);continue}
    if(command==="define"){
      const physical=[line];
      while(/\\\s*$/.test(physical.at(-1))&&lineIndex+1<lines.length)physical.push(lines[++lineIndex]);
      if(!parentActive())continue;
      const logical=physical.map((value,index)=>index===0?String(directive[2]??""):value).join("\n").replace(/\\\s*\n/g,"\n").trim(),macro=/^([A-Za-z_]\w*)(?![ \t]*\()(?:(?:[ \t]+)([\s\S]*))?$/.exec(logical);
      if(macro){definitions.set(macro[1],String(macro[2]??"").replace(/\/\*[\s\S]*?\*\//g," ").replace(/\/\/.*$/gm,"").trim());output.push(...physical);continue}
      output.push(...physical);continue;
    }
    if(command==="undef"){if(parentActive())definitions.delete(argument);continue}
  }
  const activeSource=output.join("\n");
  return{source:expandObjectMacros?replaceObjectMacros(activeSource,definitions):activeSource,definitions};
}
function macroConfiguredRegistrations(source,file,stringConstants={}){
  const functionRegistrations=functionMacroRegistrationSource(source),registrations=[...modelRegistrations([source,functionRegistrations].filter(Boolean).join("\n"),file,stringConstants),...metaModuleRegistrations(source,file)],macros=objectMacroDefinitions(source);
...
function templateContract(source,moduleClass){const name=baseTypeName(moduleClass),match=new RegExp(`template\\s*<([^>]+)>\\s*(?:struct|class)\\s+${name}\\b`).exec(source),open=moduleClass.indexOf("<"),close=moduleClass.lastIndexOf(">");if(!match||open<0||close<open)return null;const parameters=splitArguments(match[1]).map(value=>/([A-Za-z_]\w*)\s*$/.exec(value)?.[1]).filter(Boolean),argumentsList=splitArguments(moduleClass.slice(open+1,close)),constants={};parameters.forEach((parameter,index)=>{const value=numberLiteral(argumentsList[index],Number.NaN);if(Number.isFinite(value))constants[parameter]=value});return {declaration:`template <${match[1].trim()}>`,parameters,arguments:argumentsList,constants}}
function classDeclarationMatch(source,className,requiresBase=false){
  const name=baseTypeName(className),pattern=new RegExp(`\\b(?:struct|class)\\s+${name}(?:\\s+final)?${requiresBase?"\\s*:[^{]+":"(?:\\s*:[^{]+)?"}\\s*\\{`,"g");
  for(const match of source.matchAll(pattern)){
    const prefix=source.slice(Math.max(0,match.index-80),match.index);
    if(!/\benum\s*$/.test(prefix))return match
  }
  return null
}
function structBody(source,className){const match=classDeclarationMatch(source,className,true);if(!match)return null;const start=source.indexOf("{",match.index),depthStart=start;let depth=0,quote="",lineComment=false,blockComment=false;for(let index=start;index<source.length;index++){const current=source[index],next=source[index+1];if(lineComment){if(current==="\n")lineComment=false;continue}if(blockComment){if(current==="*"&&next==="/"){blockComment=false;index++}continue}if(quote){if(current==="\\"){index++;continue}if(current===quote)quote="";continue}if(current==="/"&&next==="/"){lineComment=true;index++;continue}if(current==="/"&&next==="*"){blockComment=true;index++;continue}if(current==='"'||current==="'"){quote=current;continue}if(current==="{")depth++;else if(current==="}"&&--depth===0)return source.slice(depthStart+1,index)}return null}
function enclosingNamespaces(source,className){const classIndex=classDeclarationMatch(source,className)?.index;if(classIndex===undefined)return[];const active=[],braces=[];for(let index=0;index<classIndex;index++){if(source[index]==="{"){const prefix=source.slice(Math.max(0,index-240),index),namespace=/(?:inline\s+)?namespace\s+([A-Za-z_]\w*(?:::[A-Za-z_]\w*)*)\s*$/.exec(prefix),names=namespace?namespace[1].split("::"):[];braces.push(names);active.push(...names)}else if(source[index]==="}"){const closed=braces.pop()??[];if(closed.length)active.splice(-closed.length)}}return active}
function removeClassDefinition(source,className){const match=classDeclarationMatch(source,className);if(!match)return source;const open=source.indexOf("{",match.index),close=matchingBrace(source,open);if(close<0)return source;let start=match.index,end=close+1;const prefix=source.slice(Math.max(0,start-500),start),template=/template\s*<[^;{}]+>\s*$/.exec(prefix);if(template)start=Math.max(0,start-500)+template.index;while(/\s/.test(source[end]??""))end++;if(source[end]===";")end++;return source.slice(0,start)+source.slice(end)}
function classDefinitionSource(source,className){const match=classDeclarationMatch(source,className);if(!match)return"";const open=source.indexOf("{",match.index),close=matchingBrace(source,open);if(close<0)return"";let start=match.index,end=close+1;const prefix=source.slice(Math.max(0,start-500),start),template=/template\s*<[^;{}]+>\s*$/.exec(prefix);if(template)start=Math.max(0,start-500)+template.index;while(/\s/.test(source[end]??""))end++;if(source[end]===";")end++;return source.slice(start,end)}
function qualifiedTypeDefinitionRecords(source){
  const code=sourceWithoutComments(source),records=[];
  for(const match of code.matchAll(/\b(?:struct|class)\s+([A-Za-z_]\w*)(?:\s+final)?(?:\s*:[^{]+)?\s*\{|\benum(?:\s+class|\s+struct)?\s+([A-Za-z_]\w*)\b[^;{]*\{/g)){
    if(match[1]&&/\benum\s*$/.test(code.slice(Math.max(0,match.index-80),match.index)))continue;
    ...
    if(close<0)continue;
    let start=match.index,end=close+1;
    const prefix=code.slice(Math.max(0,start-500),start),template=/template\s*<[^;{}]+>\s*$/.exec(prefix);
    if(template)start=Math.max(0,start-500)+template.index;
    while(/[ \t]/.test(code[end]??""))end++;
    if(code[end]===";")end++;
    const name=match[1]??match[2];
    records.push({start,end,key:`${namespaces.join("::")}::${name}`})
  }
  for(const match of code.matchAll(/\btypedef\s+[^;]+\s+([A-Za-z_]\w*)\s*;|\busing\s+([A-Za-z_]\w*)\s*=\s*[^;]+;/g)){
    const namespaces=namespaceStackAt(code,match.index);
    if(!isNamespaceScopeAt(code,match.index))continue;
    records.push({start:match.index,end:match.index+match[0].length,key:`${namespaces.join("::")}::${match[1]??match[2]}`})
  }
  return records.sort((left,right)=>left.start-right.start)
}
function preferNearestTargetEnums(source){
  const target=[...String(source??"").matchAll(/\b(?:struct|class)\s+([A-Za-z_]\w*)(?:\s+final)?(?:\s*:[^{]+)?\s*\{/g)].map(match=>{
    if(!isCodePosition(source,match.index))return null;
    const open=source.indexOf("{",match.index),close=matchingBrace(source,open);
    return close>=0&&/\bstatic\s+constexpr\s+int\s+rackWebParamCount\b/.test(source.slice(open+1,close))?{match,open,close}:null
  }).find(Boolean);
  if(!target)return source;
  const targetNamespaces=namespaceStackAt(source,target.match.index),groups=new Map;
  for(const match of String(source).matchAll(/\benum(?:\s+class|\s+struct)?\s+([A-Za-z_]\w*)\b[^;{]*\{/g)){
    if(match.index>=target.match.index||!isCodePosition(source,match.index)||!isNamespaceScopeAt(source,match.index))continue;
    const namespaces=namespaceStackAt(source,match.index);
    if(namespaces.length!==targetNamespaces.length||!namespaces.every((namespace,index)=>namespace===targetNamespaces[index]))continue;
    const open=source.indexOf("{",match.index),close=matchingBrace(source,open);
    if(close<0)continue;
    let end=close+1;while(/[ \t]/.test(source[end]??""))end++;if(source[end]===";")end++;
    const records=groups.get(match[1])??[];
    records.push({start:match.index,end});
    groups.set(match[1],records);
  }
  const removals=[];
...
function stripRackUiResidue(source,knownUiClasses=new Set){
  ...
  const ranges=[],pattern=/^[ \t]*(?:(?:inline|static|constexpr|virtual)\s+)*(?:[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*(?:\s*<[^;{}]+>)?[&*\s]+)+(?:[A-Za-z_]\w*::)*[A-Za-z_]\w*\s*\([^;{}]*\)\s*(?:const\s*)?(?:->\s*[^;{}]+)?\{/gm;
  for(const match of code.matchAll(pattern)){const owner=/\b([A-Za-z_]\w*)::[A-Za-z_]\w*\s*\(/.exec(match[0])?.[1];if(!isNamespaceScopeAt(code,match.index)||!(rackUiPattern.test(match[0])||uiClasses.has(owner)))continue;const open=code.indexOf("{",match.index),close=matchingBrace(code,open);if(close>=0)ranges.push([match.index,close+1])}
  for(const match of code.matchAll(/^[ \t]*[^;{}\n]+\([^;{}\n]*\)\s*(?:const\s*)?;[ \t]*(?:\n|$)/gm))if(isNamespaceScopeAt(code,match.index)&&rackUiPattern.test(match[0]))ranges.push([match.index,match.index+match[0].length]);
...
function dedupeOutOfLineMethodDefinitions(source){
  const code=sourceWithoutComments(source),seen=new Set,ranges=[],typeNamespaces=new Map,patterns=[
    /^[ \t]*(?:template\s*<[^>;{}]*>\s*)?(?!template\b)(?:(?:inline|static|constexpr|virtual)\s+)*(?:[A-Za-z_~]\w*(?:::[A-Za-z_]\w*)*(?:\s*<[^>;{}]+>)?[&*\s]+)+(?:[A-Za-z_]\w*(?:\s*<[^>;{}]+>)?::)+(?:~?[A-Za-z_]\w*)\s*\([^;{}]*\)\s*(?:const\s*)?(?:noexcept\s*)?(?:->\s*[^;{}]+)?\{/gm,
    /^[ \t]*(?:template\s*<[^>;{}]*>\s*)?(?!template\b)(?:[A-Za-z_]\w*(?:\s*<[^>;{}]+>)?::)+(?:~?[A-Za-z_]\w*)\s*\([^;{}]*\)\s*(?:noexcept\s*)?(?:\s*:\s*[^{]+)?\{/gm,
  ];
  for(const declaration of code.matchAll(/\b(?:struct|class)\s+([A-Za-z_]\w*)\b/g)){
    const namespaces=namespaceStackAt(code,declaration.index);
    if(!isNamespaceScopeAt(code,declaration.index))continue;
    if(!typeNamespaces.has(declaration[1]))typeNamespaces.set(declaration[1],new Set);
    typeNamespaces.get(declaration[1]).add(namespaces.join("::"));
  }
  for(const pattern of patterns)for(const match of code.matchAll(pattern)){
    const open=code.indexOf("{",match.index),close=matchingBrace(code,open);
    if(close<0)continue;
    const signature=match[0].slice(0,-1).replace(/\s+/g," ").trim(),canonicalSignature=signature.replace(/^(?:(?:inline|static|constexpr|virtual|[A-Z][A-Z0-9_]*)\s+)+/,""),ownerChain=/((?:[A-Za-z_]\w*(?:\s*<[^>;{}]+>)?::)+)(?:~?[A-Za-z_]\w*)\s*\(/.exec(canonicalSignature)?.[1]??"",root=/^[A-Za-z_]\w*/.exec(ownerChain)?.[0],declaredNamespaces=root?typeNamespaces.get(root):null,currentNamespace=namespaceStackAt(code,match.index).join("::"),canonicalNamespace=declaredNamespaces?.size===1?[...declaredNamespaces][0]:currentNamespace,key=`${canonicalNamespace}:${canonicalSignature}`;
    if(seen.has(key))ranges.push([match.index,close+1]);else seen.add(key);
  }
  const merged=[];
...
function normalizeConditionalTemplateImplementations(source){
  const pattern=/^[ \t]*template\s*<([^>;{}]*)>\s*(?:(?:inline|static|constexpr)\s+)*(?:[A-Za-z_~]\w*(?:::[A-Za-z_]\w*)*(?:\s*<[^;{}]+>)?[&*\s]+)+([A-Za-z_]\w*)\s*<[^;{}]+>\s*::(?:~?[A-Za-z_]\w*)\s*\([^;{}]*\)\s*(?:const\s*)?(?:noexcept\s*)?\{/gm;
  for(let pass=0;pass<32;pass++){
    const code=sourceWithoutComments(source),records=[];
    for(const match of code.matchAll(pattern)){
      const open=code.indexOf("{",match.index),close=matchingBrace(code,open);
      if(close<0)continue;
      records.push({start:match.index,end:close+1,owner:match[2],specialized:!match[1].trim(),namespaces:namespaceStackAt(code,match.index),definition:source.slice(match.index,close+1).trim()});
    }
    const owner=[...new Set(records.filter(record=>record.specialized).map(record=>record.owner))].find(candidate=>{
      ...
      return declaration&&/#\s*ifdef\s+RACK_SIMD\b/.test(declaration)&&records.some(record=>record.owner===candidate&&!record.specialized);
    });
    if(!owner)break;
    ...
    const insertAt=Math.min(...generic.map(record=>record.start)),insertionNamespaces=namespaceStackAt(code,insertAt),sameNamespace=namespaces=>namespaces.length===insertionNamespaces.length&&namespaces.every((namespace,index)=>namespace===insertionNamespaces[index]);
...
function hoistLateNumericObjectMacros(source){
  const macros=[...source.matchAll(/^[ \t]*#\s*define[ \t]+([A-Za-z_]\w*)[ \t]+([+-]?(?:0[xX][0-9a-fA-F]+|\d+)(?:[uUlLfF]*))[^\n]*$/gm)].filter(match=>new RegExp(`\\b${match[1]}\\b`).test(sourceWithoutComments(source.slice(0,match.index))));
  if(!macros.length)return source;
  const definitions=[];
  for(const match of macros.reverse()){definitions.unshift(match[0].trim());source=source.slice(0,match.index)+source.slice(match.index+match[0].length)}
...
function stripUiNamespaces(source){
  ...
  for(let pass=0;pass<64;pass++){
    ...
    const guardClose=/^\s*#endif\b[^\n]*(?:\n|$)/.exec(result.slice(end));if(guardClose)end+=guardClose[0].length;
...
function stubHostOnlyModuleMethods(body){
  ...
  while(index<body.length){
    ...
    if(blockComment){if(current==="*"&&next==="/"){blockComment=false;index+=2;continue}index++;continue}
    if(quote){if(current==="\\")index+=2;else{if(current===quote)quote="";index++}continue}
    const rawEnd=rawStringEnd(body,index);if(rawEnd>=0){index=rawEnd+1;continue}
    if(current==="/"&&next==="/"){lineComment=true;index+=2;continue}
    if(current==="/"&&next==="*"){blockComment=true;index+=2;continue}
    if(current==='"'||current==="'"){quote=current;index++;continue}
    if(current===";"){result+=body.slice(start,index+1);start=++index;continue}
    if(current==="{"){
      const close=matchingBrace(body,index);
      if(close<0)break;
      let end=close+1;
      while(/\s/.test(body[end]??""))end++;
      if(body[end]===";")end++;
      const chunk=body.slice(start,end),header=chunk.slice(0,chunk.indexOf("{"));
      result+=hostOnlyRuntimePattern.test(chunk)&&header.includes("(")?stubHostUiMethod(chunk,header):chunk;
      if(end>close+1)result+="\n";
      start=end;
      index=end;
      continue
    }
...
function replaceInlineMethodBody(source,pattern,body){const match=pattern.exec(source);if(!match)return source;const open=source.indexOf("{",match.index),close=open>=0?matchingBrace(source,open):-1;return close<0?source:source.slice(0,open+1)+body+source.slice(close)}
function appendInlineMethodStatement(source,pattern,statement){const match=pattern.exec(source);if(!match)return source;const open=source.indexOf("{",match.index),close=open>=0?matchingBrace(source,open):-1;return close<0?source:source.slice(0,close)+`\n        ${statement}\n    `+source.slice(close)}
...
function prependInlineMethodBody(source,pattern,prefix){const match=pattern.exec(source);if(!match)return source;const open=source.indexOf("{",match.index);return open<0?source:source.slice(0,open+1)+prefix+source.slice(open+1)}
...
function removeFreeFunction(source,name){const escaped=name.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),match=new RegExp(`^[ \\t]*(?:(?:inline|static)\\s+)*(?:void|bool|int|float|double|std::string)\\s+${escaped}\\s*\\([^;{}]*\\)\\s*\\{`,"m").exec(source);if(!match)return source;const open=source.indexOf("{",match.index),close=open>=0?matchingBrace(source,open):-1;return close<0?source:source.slice(0,match.index)+source.slice(close+1)}
function removeQualifiedFreeFunction(source,name){const escaped=name.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),match=new RegExp(`^[ \\t]*(?:(?:inline|static|constexpr|const)\\s+)*(?:[A-Za-z_]\\w*(?:::[A-Za-z_]\\w*)*(?:\\s*<[^;{}]+>)?[&*\\s]+)+${escaped}\\s*\\([^;{}]*\\)\\s*\\{`,"m").exec(source);if(!match)return source;const open=source.indexOf("{",match.index),close=open>=0?matchingBrace(source,open):-1;return close<0?source:source.slice(0,match.index)+source.slice(close+1)}
...
function paramQuantityHelpers(source,targetBody){const found=[];for(const match of source.matchAll(/\bstruct\s+([A-Za-z_]\w*)\s*:\s*(?:(?:public|protected|private)\s+)?(?:[A-Za-z_]\w*::)*ParamQuantity\s*\{/g)){if(!isCodePosition(source,match.index)||!isNamespaceScopeAt(source,match.index)||!new RegExp(`\\b${match[1]}\\b`).test(targetBody))continue;const open=source.indexOf("{",match.index),close=matchingBrace(source,open);if(close<0)continue;let end=close+1;while(/\s/.test(source[end]??""))end++;if(source[end]===";")end++;found.push({name:match[1],source:source.slice(match.index,end)})}return found}
function modulePrelude(source,className){const name=baseTypeName(className),match=new RegExp(`\\b(?:struct|class)\\s+${name}(?:\\s+final)?\\s*:[^{]+\\{`).exec(source);if(!match)return"";let prelude=source.slice(0,match.index).replace(/^\s*(?:\/\/\s*)?#(?:include|pragma|define)\b.*$/gm,matched=>/^\s*#define\b/.test(matched)?matched:"").trim(),namespaces=enclosingNamespaces(source,className),combined=namespaces.join("::");if(combined)prelude=prelude.replace(new RegExp(`\\bnamespace\\s+${combined.replace(/[.*+?^${}()|[\\]\\\\]/g,"\\$&")}\\s*\\{`),"");for(const namespaceName of namespaces)prelude=prelude.replace(new RegExp(`\\bnamespace\\s+${namespaceName}\\s*\\{`),"");const unclosedAnonymous=[];for(const namespaceMatch of prelude.matchAll(/\bnamespace\s*\{/g)){const open=prelude.indexOf("{",namespaceMatch.index);if(matchingBrace(prelude,open)<0)unclosedAnonymous.push([namespaceMatch.index,open+1])}for(const [start,end] of unclosedAnonymous.reverse())prelude=prelude.slice(0,start)+prelude.slice(end);if(String(className).includes("<"))prelude=prelude.replace(/template\s*<[^>]+>\s*$/s,"").trim();let depth=0,quote="",lineComment=false,blockComment=false;for(let index=0;index<prelude.length;index++){const current=prelude[index],next=prelude[index+1];if(lineComment){if(current==="\n")lineComment=false;continue}if(blockComment){if(current==="*"&&next==="/"){blockComment=false;index++}continue}if(quote){if(current==="\\")index++;else if(current===quote)quote="";continue}if(current==="/"&&next==="/"){lineComment=true;index++;continue}if(current==="/"&&next==="*"){blockComment=true;index++;continue}if(current==='"'||current==="'"){quote=current;continue}if(current==="{")depth++;else if(current==="}"&&--depth<0)return""}if(depth!==0)return"";const adapted=adaptDailyFortuneHost(prelude);return name==="ArcaneBase"?dedupeDailyFortunePrelude(adapted,"ariaSalvatriceArcaneSingletonOwned getCurrentFortuneDate"):adapted}
function namespacedModulePrelude(source,className){
  const name=baseTypeName(className),match=new RegExp(`\\b(?:struct|class)\\s+${name}(?:\\s+final)?\\s*:[^{]+\\{`).exec(source);
  if(!match)return"";
  let prelude=source.slice(0,match.index).replace(/^\s*(?:\/\/\s*)?#(?:include|pragma|define)\b.*$/gm,matched=>/^\s*#define\b/.test(matched)?matched:"").trim();
  if(String(className).includes("<"))prelude=prelude.replace(/template\s*<[^>]+>\s*$/s,"").trim();
...
function referencedPluginGlobalParts(sourceDir,analysis){
  ...
  const source=fs.readFileSync(file,"utf8"),repositoryFiles=filesOutsideNestedRepositories(sourceDir),implementationFile=path.join(sourceDir,"src","plugin.cpp"),implementationSource=fs.existsSync(implementationFile)?fs.readFileSync(implementationFile,"utf8"):"",declarations=[],typeAliases=[],implementations=[];let typeReference=analysis;
  if(/^\s*using\s+namespace\s+std\s*;/m.test(source))declarations.push("using namespace std;");
  for(let pass=0;pass<16;pass++){
    let changed=false;
    for(const match of source.matchAll(/\b(?:typedef\s+[^;]+\s+([A-Za-z_]\w*)|using\s+([A-Za-z_]\w*)\s*=\s*[^;]+)\s*;/g)){
      const name=match[1]??match[2],namespaces=namespaceStackAt(source,match.index);
      if(!isNamespaceScopeAt(source,match.index)||!new RegExp(`\\b${name}\\b`).test(typeReference)||typeAliases.some(declaration=>new RegExp(`\\b${name}\\b`).test(declaration)))continue;
      let declaration=match[0].trim();
      if(namespaces.length)declaration=`${namespaces.map(namespace=>`namespace ${namespace} {`).join("\n")}\n${declaration}\n${namespaces.map(()=>"}").join("\n")}`;
      typeAliases.push(declaration);typeReference+=`\n${declaration}`;changed=true
    }
    if(!changed)break
  }
  for(const match of source.matchAll(/\b(?:struct|class)\s+([A-Za-z_]\w*)(?:\s+final)?(?:\s*:[^{]+)?\s*\{/g)){
    const name=match[1],namespaces=namespaceStackAt(source,match.index);
    if(/\benum\s*$/.test(source.slice(Math.max(0,match.index-16),match.index)))continue;
    if(!isNamespaceScopeAt(source,match.index)||!new RegExp(`\\b${name}\\b`).test(typeReference)||new RegExp(`\\b(?:struct|class)\\s+${name}\\b`).test(analysis))continue;
    let declaration=classDefinitionSource(source,name);
    if(!declaration||rackUiPattern.test(declaration))continue;
    if(namespaces.length)declaration=`${namespaces.map(namespace=>`namespace ${namespace} {`).join("\n")}\n${declaration}\n${namespaces.map(()=>"}").join("\n")}`;
    declarations.push(declaration);typeReference+=`\n${declaration}`;
    implementations.push(...outOfLineDefinitions(implementationSource,name,true,namespaces).filter(definition=>!rackUiPattern.test(definition)))
  }
  declarations.push(...typeAliases);
  const constantSources=[source,...repositoryFiles.filter(candidate=>candidate!==file&&/\.(?:h|hh|hpp)$/.test(candidate)).map(candidate=>fs.readFileSync(candidate,"utf8"))],
    repositoryMacroNames=new Set(constantSources.flatMap(constantSource=>[...constantSource.matchAll(/^[ \t]*#\s*define[ \t]+([A-Za-z_]\w*)\b/gm)].map(match=>match[1])));
  const collectConstants=reference=>{for(const constantSource of constantSources)for(const match of constantSource.matchAll(/^[ \t]*(?:(?:inline|static)\s+)*(?:constexpr\s+|const\s+)[^\n;{}=]+?\s+([A-Za-z_]\w*)\s*=\s*[^;{}]+;/gm)){
    const name=match[1],namespaces=namespaceStackAt(constantSource,match.index),
      qualifiedReference=namespaces.some((_,index)=>new RegExp(`\\b${namespaces.slice(index).join("::")}::${name}\\b`).test(reference));
    if(!isNamespaceScopeAt(constantSource,match.index)||!new RegExp(`\\b${name}\\b`).test(reference)||(repositoryMacroNames.has(name)&&!qualifiedReference)||new RegExp(`(?:\\b(?:constexpr|const)\\b[^;\\n]*\\b${name}\\b\\s*=|^[ \\t]*#\\s*define[ \\t]+${name}\\b)`,"m").test(analysis))continue;
    let declaration=match[0].trim();
    if(namespaces.length)declaration=`${namespaces.map(namespace=>`namespace ${namespace} {`).join("\n")}\n${declaration}\n${namespaces.map(()=>"}").join("\n")}`;
    declarations.push(declaration)
  }};
  collectConstants(analysis);
  for(const match of source.matchAll(/\benum(?:\s+class|\s+struct)?(?:\s+([A-Za-z_]\w*))?\s*\{/g)){
    const open=source.indexOf("{",match.index),close=matchingBrace(source,open);if(close<0)continue;
    let end=close+1;while(/\s/.test(source[end]??""))end++;if(source[end]===";")end++;
    const declaration=source.slice(match.index,end),enumName=match[1],names=splitArguments(source.slice(open+1,close)).map(entry=>/^\s*([A-Za-z_]\w*)/.exec(entry)?.[1]).filter(Boolean);
    const referenced=Boolean(enumName&&new RegExp(`\\b${enumName}\\b`).test(analysis))||names.some(name=>new RegExp(`\\b${name}\\b`).test(analysis)),alreadyDeclared=Boolean(enumName&&new RegExp(`\\benum(?:\\s+class|\\s+struct)?\\s+${enumName}\\b`).test(analysis))||names.some(name=>new RegExp(`\\benum[^{}]*\\{[^}]*\\b${name}\\b`,"s").test(analysis));
    if(referenced&&!alreadyDeclared)declarations.push(declaration)
  }
  const defaults={madzineDefaultContrast:"255.f",madzineDefaultTheme:"-1"};
  const externSources=[source,...repositoryFiles.filter(candidate=>candidate!==file).map(candidate=>fs.readFileSync(candidate,"utf8"))];
  for(const externSource of externSources)for(const match of externSource.matchAll(/^\s*extern\s+(.+?)\s+([*&]*)([A-Za-z_]\w*)\s*((?:\[[^\]\n]*\])+)?\s*;[^\n]*$/gm)){
    const type=`${match[1].trim()}${match[2]}`,name=match[3],extent=match[4]??"";
    if(!new RegExp(`\\b${name}\\b`).test(analysis)||/\b(?:Model|Plugin)\s*\*$/.test(type)||namespaceGlobalDefinitions(analysis,name))continue;
    const lockedDefinition=repositoryFiles.map(candidate=>namespaceGlobalDefinitions(fs.readFileSync(candidate,"utf8"),name)).find(Boolean),cLinkage=[...externSource.slice(0,match.index).matchAll(/\bextern\s+"C"\s*\{/g)].some(wrapper=>{const open=externSource.indexOf("{",wrapper.index),close=matchingBrace(externSource,open);return close>match.index}),linkage=cLinkage?'extern "C" ':"";
    declarations.push(lockedDefinition|| (extent?`${linkage}inline ${type} ${name}${extent}{};`:`${linkage}inline ${type} ${name} = ${defaults[name]??"{}"};`))
  }
  ...
  const helperImplementationSource=/\bvoid\s+init\s*\(\s*(?:rack::)?Plugin\s*\*/.test(implementationSource)?removeFreeFunction(implementationSource,"init"):implementationSource,functionNames=[...new Set([source,helperImplementationSource].flatMap(candidate=>[...candidate.matchAll(/^[ \t]*(?:(?:inline|static|constexpr|consteval)\s+)*(?:[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*(?:\s*<[^;{}]+>)?[&*\s]+)+([A-Za-z_]\w*)\s*\(/gm)].filter(match=>isNamespaceScopeAt(candidate,match.index)).map(match=>match[1])))],selected=new Set,selectedDefinitions=new Map;
  let expanded=[analysis,...implementations].join("\n");
  for(let pass=0;pass<functionNames.length;pass++){
    let changed=false;
    for(const name of functionNames){
      if(selected.has(name)||!new RegExp(`\\b${name}\\b`).test(expanded)||outOfLineFreeFunctionDefinitions(expanded,name).length)continue;
      const definitions=[...outOfLineFreeFunctionDefinitions(source,name,true),...outOfLineFreeFunctionDefinitions(helperImplementationSource,name,true)].filter(definition=>!rackUiPattern.test(definition)&&!/\bNVG(?:color|context)\b|\bnvg[A-Z]\w*\s*\(/.test(definition)&&!/\bAPP\s*->\s*(?!engine\b)/.test(definition));
      if(!definitions.length)continue;
      selected.add(name);changed=true;const joined=definitions.join("\n\n");selectedDefinitions.set(name,joined);expanded+=`\n${joined}`;
    }
    if(!changed)break
  }
  for(const match of source.matchAll(/^[ \t]*(?:(?:inline|static|constexpr|consteval|const|volatile|unsigned|signed|long|short)\s+)*(?:[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*(?:\s*<[^;{}]+>)?[&*\s]+)+([A-Za-z_]\w*)\s*\([^;{}]*\)\s*(?:const\s*)?(?:noexcept\s*)?;/gm)){
    const name=match[1],namespaces=namespaceStackAt(source,match.index);
    if(!selected.has(name)||!isNamespaceScopeAt(source,match.index))continue;
    let declaration=match[0].trim();
    if(namespaces.length)declaration=`${namespaces.map(namespace=>`namespace ${namespace} {`).join("\n")}\n${declaration}\n${namespaces.map(()=>"}").join("\n")}`;
    declarations.push(declaration)
  }
  collectConstants(expanded);
  for(const name of functionNames)if(selectedDefinitions.has(name))implementations.push(selectedDefinitions.get(name));
...
function sourceWithoutIncludes(source){
  return source.replace(/^\s*(?:\/\/\s*)?#(?:include|pragma|define)\b.*$/gm,matched=>{
    if(/^\s*#define\b/.test(matched))return matched;
    return""
  }).replace(/(?:inline\s+)?[\w:<>&*\s]+\([^;{}]*\)\s*\{(?=[^{}]*\bAPP\s*->\s*(?!engine\b)[A-Za-z_]\w*)[^{}]*\}/g,"").trim()
}
function flattenExternCWrappers(source){return source.replace(/^\s*#\s*ifdef\s+__cplusplus\s*\n\s*extern\s+"C"\s*\n?\s*\{\s*\n\s*#\s*endif\s*$/gm,"").replace(/^\s*#\s*ifdef\s+__cplusplus\s*\n\s*\}\s*\n\s*#\s*endif\s*$/gm,"")}
...
function dedupeRepeatedTopLevelTypes(source){const seen=new Set,removals=[],pattern=/\b(?:struct|class)\s+([A-Za-z_]\w*)(?:\s+final)?(?:\s*:[^{]+)?\s*\{/g;for(const match of source.matchAll(pattern)){if(/\benum\s*$/.test(source.slice(Math.max(0,match.index-16),match.index)))continue;if(new RegExp(`\\b${match[1]}\\s*::`).test(match[0]))continue;const namespaces=namespaceStackAt(source,match.index);if(!isNamespaceScopeAt(source,match.index))continue;const key=`${canonicalNamespaceKey(namespaces)}:${match[1]}`;if(!seen.has(key)){seen.add(key);continue}const open=source.indexOf("{",match.index),close=matchingBrace(source,open);if(close<0)continue;let end=close+1;while(/[ \t]/.test(source[end]??""))end++;if(source[end]===";")end++;removals.push([match.index,end])}let result=source;for(const [start,end] of removals.reverse())result=result.slice(0,start)+result.slice(end);return result}
function dedupeRepeatedTopLevelEnums(source){const seen=new Set,removals=[],pattern=/\benum(?:\s+class|\s+struct)?\s+([A-Za-z_]\w*)\b[^;{]*\{/g;for(const match of source.matchAll(pattern)){const namespaces=namespaceStackAt(source,match.index);if(!isNamespaceScopeAt(source,match.index))continue;const open=source.indexOf("{",match.index),close=matchingBrace(source,open);if(close<0)continue;let end=close+1;while(/[ \t]/.test(source[end]??""))end++;if(source[end]===";")end++;const declaration=source.slice(match.index,end).replace(/\s+/g," ").trim(),key=`${canonicalNamespaceKey(namespaces)}:${match[1]}:${declaration}`;if(!seen.has(key)){seen.add(key);continue}removals.push([match.index,end])}let result=source;for(const [start,end] of removals.reverse())result=result.slice(0,start)+result.slice(end);return result}
function dedupeRepeatedNamespaceVariables(source){const code=sourceWithoutComments(source),seen=new Set,removals=[],pattern=/^[ \t]*(?!(?:extern|struct|class|enum|union|namespace|using|typedef|template)\b)(?:(?:static|inline|constexpr|const|volatile|thread_local|unsigned|signed|long|short)\s+)*(?:[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*(?:\s*<[^;{}]+>)?)(?:\s*[*&]\s*|\s+)([A-Za-z_]\w*)\s*(?:\[[^\]]*\]\s*)*(?=\s*(?:=|\{|;))/gm;for(const match of code.matchAll(pattern)){if(!isCodePosition(code,match.index)||!isNamespaceScopeAt(code,match.index))continue;const end=namespaceStatementEnd(code,match.index+match[0].length);if(end<0)continue;const namespaces=namespaceStackAt(code,match.index),key=`${namespaces.join("::")}:${match[1]}`;if(seen.has(key))removals.push([match.index,end]);else seen.add(key)}let result=source;for(const [start,end] of removals.reverse())result=result.slice(0,start)+result.slice(end);return result}
function dedupeRepeatedNamespaceConstants(source){const seen=new Set,removals=[],pattern=/^[ \t]*(?:(?:inline|static)\s+)*(?:constexpr\s+|const\s+)[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*(?:\s*<[^;\n]+>)?\s+([A-Za-z_]\w*)\s*=\s*([^;\n]+);[ \t]*$/gm;for(const match of source.matchAll(pattern)){const namespaces=namespaceStackAt(source,match.index);if(!isNamespaceScopeAt(source,match.index))continue;const declaration=match[0].replace(/\s+/g," ").trim(),key=`${namespaces.join("::")}:${match[1]}:${declaration}`;if(!seen.has(key)){seen.add(key);continue}removals.push([match.index,match.index+match[0].length])}let result=source;for(const [start,end] of removals.reverse())result=result.slice(0,start)+result.slice(end);return result}
...
function namespaceUsingPrelude(source){
  const importsRack=/^\s*using\s+namespace\s+rack\s*;/m.test(source),
    names=[...new Set([...source.matchAll(/^\s*using\s+namespace\s+([A-Za-z_]\w*(?:::[A-Za-z_]\w*)*)\s*;/gm)]
      .map(match=>importsRack&&match[1]==="dsp"?"rack::dsp":match[1])
...
function namespaceSpecificUsingDeclarations(source){const declarations=[];for(const match of source.matchAll(/^[ \t]*using\s+([A-Za-z_]\w*(?:::[A-Za-z_]\w*)+)\s*;/gm)){const namespaces=namespaceStackAt(source,match.index);if(!isNamespaceScopeAt(source,match.index))continue;let declaration=match[0].trim();if(namespaces.length)declaration=`${namespaces.map(namespace=>`namespace ${namespace} {`).join("\n")}\n${declaration}\n${namespaces.map(()=>"}").join("\n")}`;declarations.push(declaration)}return[...new Set(declarations)]}
...
function declaredTypeNames(source){const names=[...source.matchAll(/\b(?:struct|class)\s+([A-Za-z_]\w*)\b/g)].map(match=>match[1]);for(const match of source.matchAll(/\benum(?:\s+class|\s+struct)?\s+([A-Za-z_]\w*)\b/g))names.push(match[1]);for(const match of source.matchAll(/\btypedef\s+[^;]+\s+([A-Za-z_]\w*)\s*;/g))names.push(match[1]);for(const match of source.matchAll(/\busing\s+([A-Za-z_]\w*)\s*=/g))names.push(match[1]);return[...new Set(names)]}
function enumDeclarationSource(source,name){const escaped=String(name).replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),match=new RegExp(`\\benum(?:\\s+class|\\s+struct)?\\s+${escaped}\\b[^;{]*\\{`).exec(source);if(!match)return"";const open=source.indexOf("{",match.index),close=matchingBrace(source,open);if(close<0)return"";let end=close+1;while(/[ \t]/.test(source[end]??""))end++;if(source[end]===";")end++;return source.slice(match.index,end)}
function referencedGlobalEnumDeclarations(sourceFiles,reference){
  ...
  for(const file of sourceFiles){
    ...
    for(const match of source.matchAll(/\benum(?:\s+class|\s+struct)?\s+([A-Za-z_]\w*)\b[^;{]*\{/g)){
      const namespaces=namespaceStackAt(source,match.index);
      if(!isNamespaceScopeAt(source,match.index))continue;
      const info=enumInfo(source,match[1]);
      if(!info?.identifiers?.some(identifier=>new RegExp(`\\b${identifier.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}\\b`).test(reference)))continue;
      let declaration=enumDeclarationSource(source,match[1]);
      if(namespaces.length)declaration=`${namespaces.map(namespace=>`namespace ${namespace} {`).join("\n")}\n${declaration}\n${namespaces.map(()=>"}").join("\n")}`;
      declarations.push(declaration);
    }
  }
...
function anonymousTypedefDeclarationSource(source,name){const escaped=String(name).replace(/[.*+?^${}()|[\]\\]/g,"\\$&");for(const match of source.matchAll(/\btypedef\s+(?:struct|class|union|enum)\s*\{/g)){const open=source.indexOf("{",match.index),close=matchingBrace(source,open);if(close<0)continue;const suffix=new RegExp(`^\\s*${escaped}\\s*;`).exec(source.slice(close+1));if(suffix)return source.slice(match.index,close+1+suffix[0].length)}return""}
function typeDeclarationSource(source,name){const escaped=String(name).replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),plain=classDefinitionSource(source,name),enumeration=enumDeclarationSource(source,name),anonymousTypedef=anonymousTypedefDeclarationSource(source,name),typedef=new RegExp(`\\btypedef\\s+[^;]+\\s+${escaped}\\s*;`).exec(source)?.[0],using=new RegExp(`\\busing\\s+${escaped}\\s*=\\s*[^;]+;`).exec(source)?.[0];return[plain,enumeration,anonymousTypedef,typedef,using].filter(Boolean).join("\n")}
function declaredDependencyNames(source){
  ...
  const names=[],candidates=[],collect=pattern=>{for(const match of source.matchAll(pattern))candidates.push({index:match.index,name:match[1]})};
  for(const pattern of [
    /\b(?:struct|class)\s+([A-Za-z_]\w*)\b(?:\s+final)?(?=\s*(?::|\{))/g,
    /\benum(?:\s+class|\s+struct)?\s+([A-Za-z_]\w*)\b(?=\s*(?::[^{;]+)?\s*\{)/g,
    /\btypedef\s+[^;]+\s+([A-Za-z_]\w*)\s*;/g,
    /\busing\s+([A-Za-z_]\w*)\s*=/g,
    /^\s*(?:(?:inline|static|constexpr|virtual|explicit)\s+)*(?:[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*(?:\s*<[^;{}]+>)?[&*\s]+)+([A-Za-z_]\w*)\s*\(/gm,
    /^\s*(?:(?:inline|static)\s+)*constexpr\s+[^;=]+\s+([A-Za-z_]\w*)\s*=/gm,
    /^\s*(?:(?:extern|static)\s+)?const\s+[^;=]+?\s+([A-Za-z_]\w*)\s*(?:\[[^\]]*\])?\s*(?:=|;)/gm,
    /^\s*(?:(?:extern|static|inline|constexpr|const)\s+)*(?:[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*)\s*[*&\s]+([A-Za-z_]\w*)\s*(?:\[[^\]]*\])+\s*(?:=|;)/gm,
    /^\s*(?:(?:extern|static|inline|constexpr|const|volatile|thread_local)\s+)*(?:[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*)(?:\s*<[^;{}]+>)?\s*(?:[*&]\s*)?([A-Za-z_]\w*)\s*(?:=[^;]+|,\s*[^;]+)?;/gm,
  ])collect(pattern);
  candidates.sort((left,right)=>left.index-right.index);
  let cursor=0,depth=0,namespaceDepth=0,quote="";
  const namespaceBraces=[];
  for(const candidate of candidates){
    for(;cursor<candidate.index;cursor++){
      const current=source[cursor];
      if(quote){if(current==="\\")cursor++;else if(current===quote)quote="";continue}
      const rawEnd=rawStringEnd(source,cursor);if(rawEnd>=0){cursor=rawEnd;continue}
      if(current==='"'||current==="'"){quote=current;continue}
      if(current==="{"){
        const prefix=source.slice(Math.max(0,cursor-200),cursor),isNamespace=/(?:^|[;{}\n])\s*(?:inline\s+)?namespace(?:\s+[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*)?\s*$/.test(prefix);
        namespaceBraces.push(isNamespace);depth++;if(isNamespace)namespaceDepth++;
      }else if(current==="}"){
        const isNamespace=namespaceBraces.pop()??false;if(isNamespace)namespaceDepth=Math.max(0,namespaceDepth-1);depth=Math.max(0,depth-1)
      }
    }
    if(depth===namespaceDepth)names.push(candidate.name)
  }
  for(const match of source.matchAll(/\btypedef\s+(?:struct|class|union|enum)\s*\{/g)){const open=source.indexOf("{",match.index),close=matchingBrace(source,open),name=/^\s*([A-Za-z_]\w*)\s*;/.exec(source.slice(close+1))?.[1];if(close>=0&&name)names.push(name)}
  return[...new Set(names)].filter(name=>!languageAndLibraryTypes.has(name))
}
function stripTopLevelDefaultArgument(argument){const stack=[],pairs={"(":")","{":"}","[":"]","<":">"};let quote="";for(let index=0;index<argument.length;index++){const current=argument[index];if(quote){if(current==="\\")index++;else if(current===quote)quote="";continue}if(current==='"'||current==="'"){quote=current;continue}if(Object.hasOwn(pairs,current))stack.push(pairs[current]);else if(stack.at(-1)===current)stack.pop();else if(current==="="&&!stack.length&&argument[index-1]!=="="&&argument[index+1]!=="=")return argument.slice(0,index).trimEnd()}return argument}
function stripFunctionDefaultArguments(signature){const open=signature.indexOf("("),close=open>=0?matchingParenthesis(signature,open):-1;if(open<0||close<0)return signature;const parameters=splitArguments(signature.slice(open+1,close)).map(stripTopLevelDefaultArgument).join(", ");return`${signature.slice(0,open+1)}${parameters}${signature.slice(close)}`}
function stripRepeatedDefaultArgumentsOnDefinitions(source){const code=sourceWithoutComments(source),ranges=[],canonical=signature=>stripFunctionDefaultArguments(signature).replace(/\s+/g," ").replace(/\s*([(),*&<>])\s*/g,"$1").trim(),definitionPattern=/^[ \t]*(?:(?:inline|static|constexpr|consteval|const|volatile|unsigned|signed|long|short)\s+)*(?:[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*(?:\s*<[^;{}]+>)?[&*\s]+)+([A-Za-z_]\w*)\s*\([^;{}]*\)\s*(?:const\s*)?(?:noexcept\s*)?\{/gm;for(const match of code.matchAll(definitionPattern)){if(!isCodePosition(code,match.index)||!isNamespaceScopeAt(code,match.index))continue;const open=code.indexOf("{",match.index),signature=source.slice(match.index,open).trim();if(!signature.includes("="))continue;const expected=canonical(signature),name=match[1],declarationPattern=new RegExp(`^[ \\t]*[^;{}]*\\b${name.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}\\s*\\([^;{}]*\\)\\s*(?:const\\s*)?(?:noexcept\\s*)?;`,"gm"),prior=code.slice(0,match.index),declared=[...prior.matchAll(declarationPattern)].some(declaration=>declaration[0].includes("=")&&isCodePosition(code,declaration.index)&&isNamespaceScopeAt(code,declaration.index)&&canonical(source.slice(declaration.index,declaration.index+declaration[0].length-1).trim())===expected);if(declared)ranges.push([match.index,open,stripFunctionDefaultArguments(signature)])}for(const [start,end,replacement] of ranges.reverse())source=source.slice(0,start)+replacement+source.slice(end);return source}
function functionDecorationMacroDefinitions(source){
  ...
  for(const match of source.matchAll(/^[ \t]*#[ \t]*define[ \t]+([A-Za-z_]\w*)[ \t]+([^\n\\]*)[ \t]*$/gm)){
    ...
    definitions.set(match[1],[...new Set(`${previous} ${semantics}`.trim().split(/\s+/).filter(Boolean))].join(" "))
  }
  return definitions
}
...
function namespaceFunctionForwardDeclarations(source,macroSource=source){const declarations=[];for(const match of source.matchAll(/^[ \t]*(?:(?:inline|static|constexpr|consteval|const|volatile|unsigned|signed|long|short)\s+)*(?:[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*(?:\s*<[^;{}]+>)?[&*\s]+)+([A-Za-z_]\w*)\s*\([^;{}]*\)\s*(?:const\s*)?(?:noexcept\s*)?\{/gm)){if(!isCodePosition(source,match.index)||!isNamespaceScopeAt(source,match.index))continue;const open=source.indexOf("{",match.index),signature=normalizeFunctionDecorationMacros(stripFunctionDefaultArguments(source.slice(match.index,open).trim()),macroSource);if(!signature||rackUiPattern.test(signature)||/\btemplate\s*</.test(signature))continue;let declaration=`${signature};`,namespaces=namespaceStackAt(source,match.index);if(namespaces.length)declaration=`${namespaces.map(namespace=>`namespace ${namespace} {`).join("\n")}\n${declaration}\n${namespaces.map(()=>"}").join("\n")}`;declarations.push(declaration)}return[...new Set(declarations)]}
function referencedVecDspHelpers(source,reference){const helpers=[];for(const match of source.matchAll(/^[ \t]*(?:(?:inline|static|constexpr)\s+)*(?:[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*[&*\s]+)+([A-Za-z_]\w*)\s*\([^;{}]*\bVec\b[^;{}]*\)\s*\{/gm)){const name=match[1];if(!new RegExp(`\\b${name}\\s*\\(`).test(reference)||!isNamespaceScopeAt(source,match.index))continue;const open=source.indexOf("{",match.index),close=matchingBrace(source,open);if(close>=0)helpers.push(source.slice(match.index,close+1))}return[...new Set(helpers)].join("\n\n")}
function deferFreeFunctionsReferencingTypes(source,typeNames){const ranges=[],definitions=[],pattern=/^[ \t]*(?:(?:inline|static|constexpr|consteval|const|volatile|unsigned|signed|long|short)\s+)*(?:[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*(?:\s*<[^;{}]+>)?[&*\s]+)+([A-Za-z_]\w*)\s*\([^;{}]*\)\s*(?:const\s*)?(?:noexcept\s*)?\{/gm;for(const match of source.matchAll(pattern)){const namespaces=namespaceStackAt(source,match.index);if(!isNamespaceScopeAt(source,match.index))continue;const open=source.indexOf("{",match.index),close=matchingBrace(source,open);if(close<0)continue;const definition=source.slice(match.index,close+1);if(!typeNames.some(name=>new RegExp(`\\b${name.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}\\b`).test(definition)))continue;let wrapped=definition.trim();if(namespaces.length)wrapped=`${namespaces.map(namespace=>`namespace ${namespace} {`).join("\n")}\n${wrapped}\n${namespaces.map(()=>"}").join("\n")}`;definitions.push(wrapped);ranges.push([match.index,close+1])}let remaining=source;for(const [start,end] of ranges.reverse())remaining=remaining.slice(0,start)+remaining.slice(end);return{source:remaining.trim(),definitions:[...new Set(definitions)]}}
...
function referencedDependencyBundleForAdapter(sourceFiles,analysis,excludedFiles,sourceDir){
  ...
  const directlyReachableMacroNames=[
    ...sourceFiles.flatMap(file=>fs.existsSync(file)?[...fs.readFileSync(file,"utf8").matchAll(/^[ \t]*#\s*define[ \t]+([A-Za-z_]\w*)\b/gm)].map(match=>match[1]):[]),
  ...
  const directAnalysis=analysis,referencedExterns=[],referencedExternTypes=[];for(const file of sourceFiles){const source=fs.readFileSync(file,"utf8");for(const match of source.matchAll(/^\s*extern\s+(.+?)\s+([*&]*)([A-Za-z_]\w*)\s*(?:\[[^\]\n]*\])*\s*;[^\n]*$/gm))if(new RegExp(`\\b${match[3]}\\b`).test(analysis)){referencedExterns.push(match[0].trim());referencedExternTypes.push({type:baseTypeName(`${match[1]}${match[2]}`),name:match[3]})}}analysis=[analysis,...new Set(referencedExterns)].join("\n");
  ...
  const headers=sourceFiles.filter(file=>{if(!/\.(?:hpp|hh|h)$/.test(file)||["plugin.hpp","skins.hpp"].includes(path.basename(file))||excludedFiles.has(file))return false;const raw=fs.readFileSync(file,"utf8"),source=stripRackUiBlocks(flattenExternCWrappers(sourceWithoutIncludes(stripHeaderGuardOpen(raw))));return source.trim()&&(!/^\s*extern\s+(?:(?:rack::)?plugin::)?Model\s*\*/m.test(raw)||declaredDependencyNames(source).length>0||referencedLocalFreeFunctionDefinitions(source,source).length>0)}).map(file=>{const source=stripRackUiBlocks(flattenExternCWrappers(sourceWithoutIncludes(stripHeaderGuardOpen(fs.readFileSync(file,"utf8"))))),names=declaredDependencyNames(source),functionNames=[...new Set([...source.matchAll(/^[ \t]*(?:template\s*<[^>]*>\s*)?(?:(?:inline|static|constexpr|consteval|const|volatile|unsigned|signed|long|short)\s+)*(?:[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*(?:\s*<[^;{}]+>)?[&*\s]+)+([A-Za-z_]\w*)\s*\([^;{}]*\)\s*(?:const\s*)?(?:noexcept\s*)?\{/gm)].filter(match=>isCodePosition(source,match.index)&&isNamespaceScopeAt(source,match.index)).map(match=>match[1]))],namespaceNames=[...new Set([...source.matchAll(/\b(?:inline\s+)?namespace\s+([A-Za-z_]\w*)\s*\{/g)].filter(match=>isCodePosition(source,match.index)).map(match=>match[1]))],qualifiedNames=new Map([...names,...functionNames].map(name=>{const namespaces=enclosingNamespaces(source,name),identities=namespaces.map((_,index)=>[...namespaces.slice(index),name].join("::"));return[name,identities]}));return{file,source,names,functionNames,namespaceNames,qualifiedNames}}),declarations=headers.map(header=>header.source).join("\n"),selected=[],defined=new Set([...declaredDependencyNames(analysis),...[...analysis.matchAll(/^[ \t]*#\s*define[ \t]+([A-Za-z_]\w*)\b/gm)].map(match=>match[1]),...directlyReachableMacroNames,"Module","ParamQuantity","Input","Output","Light","Skins","SkinChangeListener"]),nameReferenced=(header,name,reference)=>{const escaped=value=>value.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),qualified=(header.qualifiedNames.get(name)??[]).some(identity=>new RegExp(`\\b${escaped(identity)}\\b`).test(reference)),unqualified=new RegExp(`(?:^|[^:\\w])${escaped(name)}\\b`).test(reference);return qualified||(!defined.has(name)&&unqualified)},functionReferenced=(header,name,reference)=>{const escaped=name.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),callSuffix=`${escaped}(?:\\s*<[^;{}]+>)?\\s*\\(`,qualified=[...header.source.matchAll(new RegExp(`^[ \\t]*(?:template\\s*<[^>]*>\\s*)?(?:(?:inline|static|constexpr|consteval|const|volatile|unsigned|signed|long|short)\\s+)*(?:[A-Za-z_]\\w*(?:::[A-Za-z_]\w*)*(?:\\s*<[^;{}]+>)?[&*\\s]+)+${escaped}\\s*\\(`,"gm"))].flatMap(match=>{const namespaces=namespaceStackAt(header.source,match.index);return namespaces.map((_,index)=>[...namespaces.slice(index),name].join("::"))}).some(identity=>new RegExp(`\\b${identity.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}(?:\\s*<[^;{}]+>)?\\s*\\(`).test(reference)),unqualified=new RegExp(`(?:^|[^:\\w])${callSuffix}`).test(reference);return qualified||(!defined.has(name)&&unqualified)},namespaceReferenced=(header,reference)=>header.namespaceNames.some(name=>!defined.has(name)&&new RegExp(`\\b${name.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}\\s*::`).test(reference));
  ...
  for(const match of analysis.matchAll(/\benum(?:\s+class|\s+struct)?\s+([A-Za-z_]\w*)\b[^;{]*\{/g)){
    const namespaces=namespaceStackAt(analysis,match.index);
    if(!isNamespaceScopeAt(analysis,match.index))continue;
    const declaration=enumDeclarationSource(analysis,match[1]),info=enumInfo(declaration,match[1]);
    if(!declaration||!info?.identifiers?.some(identifier=>typeof identifier==="string"&&new RegExp(`\\b${identifier.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}\\b`).test(dependencyReference)))continue;
    let wrapped=declaration;
    if(namespaces.length)wrapped=`${namespaces.map(namespace=>`namespace ${namespace} {`).join("\n")}\n${wrapped}\n${namespaces.map(()=>"}").join("\n")}`;
  ...
  const supportDirectories=new Set(selected.map(header=>path.dirname(header.file))),supportFiles=sourceFiles.filter(file=>/\.(?:c|cc|cpp|cxx)$/.test(file)&&supportDirectories.has(path.dirname(file))&&!rackUiPattern.test(fs.readFileSync(file,"utf8"))),crossFileSupport=[],knownCrossFileSupport=new Set;let crossFileReference=chunks.join("\n");for(let pass=0;pass<16;pass++){let changed=false;for(const file of supportFiles){const source=fs.readFileSync(file,"utf8");for(const definition of referencedLocalFreeFunctionDefinitions(source,crossFileReference)){if(knownCrossFileSupport.has(definition))continue;knownCrossFileSupport.add(definition);crossFileSupport.push({source,definition});crossFileReference+=`\n${definition}`;changed=true}}if(!changed)break}if(crossFileSupport.length){const declarations=[...new Set(crossFileSupport.map(({definition})=>freeFunctionDeclaration(definition)).filter(Boolean))],globals=[...new Set(crossFileSupport.map(({source})=>namespaceGlobalDefinitions(source,crossFileSupport.map(item=>item.definition).join("\n"))).filter(Boolean))];chunks.splice(implementationStart,0,...declarations,...globals);chunks.push(...crossFileSupport.map(item=>item.definition))}
...
function moduleDefinition(sourceDir,sourceFiles,registration){
  const registrationSource=fs.readFileSync(registration.file,"utf8"),quoted=[...registrationSource.matchAll(/^\s*#include\s+"([^"]+)"/gm)].map(match=>match[1]);
  ...
  for(const include of quoted)for(const candidate of [path.resolve(path.dirname(registration.file),include),path.resolve(sourceDir,include),path.resolve(sourceDir,"src",include)])if(fs.existsSync(candidate))preferred.add(candidate);
  ...
  for(const file of [...new Set(candidates)]){
    const rawSource=file===registration.definitionFile&&registration.definitionSource?registration.definitionSource:fs.readFileSync(file,"utf8");let source=rawSource,body=structBody(source,registration.moduleClass);
    if(body===null&&/^\s*#\s*(?:if|ifdef|ifndef)\b/m.test(rawSource)){const processed=preprocessMacroSource(rawSource,new Map()).source,processedBody=structBody(processed,registration.moduleClass);if(processedBody!==null){source=processed;body=processedBody}}
    if(body===null)continue;
    const namespace=enclosingNamespaces(source,registration.moduleClass);if(expectedNamespaces.some(expected=>expected.length)&&!matchesExpectedNamespace(namespace))continue;
    ...
    if(!best||score>best.score)best={file,source,body,score,namespace};
  }
  if(!best)fail(`Could not extract module class ${registration.moduleClass}`);
  const resolvedIncludes=quoted.map(include=>[path.resolve(path.dirname(registration.file),include),path.resolve(sourceDir,include),path.resolve(sourceDir,"src",include)].find(file=>fs.existsSync(file)&&fs.statSync(file).isFile())),
    definitionIncludeIndex=resolvedIncludes.findIndex(file=>file===best.file),
...
function expandClassBodyIncludes(body,file,sourceDir,macros=new Map,depth=0){if(depth>=12)return body;const root=path.resolve(sourceDir),prefix=`${root}${path.sep}`;let activeDefinitions=new Map(macros);return body.replace(/^([ \t]*)#include\s+"([^"]+)"[ \t]*(?:\/\/[^\r\n]*)?\r?$/gm,(line,indent,include)=>{const candidates=[path.resolve(path.dirname(file),include),path.resolve(root,"src",include),path.resolve(root,include)],target=candidates.find(candidate=>(candidate===root||candidate.startsWith(prefix))&&fs.existsSync(candidate)&&fs.statSync(candidate).isFile());if(!target)return line;const expanded=preprocessMacroSource(stripHeaderGuardOpen(fs.readFileSync(target,"utf8")),activeDefinitions);activeDefinitions=expanded.definitions;return expandClassBodyIncludes(expanded.source,target,sourceDir,activeDefinitions,depth+1).split("\n").map(value=>value?`${indent}${value}`:value).join("\n")})}
function inheritedDefinitions(sourceFiles,source,className){const found=[],visiting=new Set,definedNames=new Set;function visit(type){const resolvedType=resolveTypeAliases(sourceFiles,type);if(rackModuleBase(resolvedType))return;const name=baseTypeName(resolvedType),identity=resolvedType.replace(/\s/g,"");if(!name||visiting.has(identity)||definedNames.has(name))return;visiting.add(identity);let definition=null;for(const file of sourceFiles){const candidateSource=adaptDailyFortuneHost(fs.readFileSync(file,"utf8")),body=structBody(candidateSource,name);if(body!==null){const declared=declaredBases(candidateSource,name),template=templateContract(candidateSource,resolvedType),bindings=Object.fromEntries((template?.parameters??[]).map((parameter,index)=>[parameter,template.arguments[index]??parameter])),bases=declared.map(base=>substituteType(base,bindings)).map(base=>resolveTypeAliases(sourceFiles,base));definition={file,source:candidateSource,body,analysisBody:substituteType(body,bindings),name,namespace:enclosingNamespaces(candidateSource,name),actualType:resolvedType,declaredBases:declared,bases,base:bases[0],templateDeclaration:template?.declaration??""};break}}if(!definition){found.push({name,missing:true});return}if(definition.base)visit(definition.base);definition.prelude=modulePrelude(definition.source,resolvedType);definition.implementations=sourceFiles.flatMap(file=>outOfLineDefinitions(fs.readFileSync(file,"utf8"),name));const availableSupportNames=declaredTypeNames(definition.prelude),supportNames=new Set,supportImplementations=[];let supportSource=[definition.body,...definition.declaredBases].join("\n");for(let depth=0;depth<16;depth++){const nextNames=availableSupportNames.filter(supportName=>!supportNames.has(supportName)&&new RegExp(`\\b${supportName}\\b`).test(supportSource));if(!nextNames.length)break;for(const supportName of nextNames){supportNames.add(supportName);const implementations=sourceFiles.flatMap(file=>outOfLineDefinitions(fs.readFileSync(file,"utf8"),supportName));supportImplementations.push(...implementations);supportSource+=`\n${plainStructBody(definition.prelude,supportName)??""}\n${implementations.join("\n")}`}}definition.supportImplementations=[...new Set(supportImplementations)];definedNames.add(name);found.push(definition)}const template=templateContract(source,className),bindings=Object.fromEntries((template?.parameters??[]).map((parameter,index)=>[parameter,template.arguments[index]??parameter])),base=substituteType(declaredBases(source,className)[0]??"",bindings);if(base&&!rackModuleBase(base))visit(base);return found}
function plainStructBody(source,className){const match=classDeclarationMatch(source,className);if(!match)return null;const open=source.indexOf("{",match.index),close=matchingBrace(source,open);return close<0?null:source.slice(open+1,close)}
...
function localPlainStructDefinitions(source,className,targetBody,excludedNames=new Set){
  const classMatch=new RegExp(`\\b(?:struct|class)\\s+${baseTypeName(className)}\\s*:`).exec(source),prefix=classMatch?source.slice(0,classMatch.index):source,found=[];
  for(const match of prefix.matchAll(/\b(?:struct|class)\s+([A-Za-z_]\w*)(?:\s+final)?(?:\s*:[^{]+)?\s*\{/g)){
    const name=match[1];
    if(excludedNames.has(name)||!new RegExp(`\\b${name}\\b`).test(targetBody))continue;
    const namespaces=namespaceStackAt(prefix,match.index);
    if(!isNamespaceScopeAt(prefix,match.index))continue;
    const open=prefix.indexOf("{",match.index),close=matchingBrace(prefix,open);
    if(close<0)continue;
    const definition=prefix.slice(match.index,close+1)+";";
    ...
    const namespaceOpen=namespaces.map(namespace=>`namespace ${namespace} {`).join("\n"),namespaceClose=namespaces.map(()=>"}").reverse().join("\n");
...
function enumRecordInNamespace(source,names,targetNamespace=[]){
  for(const name of Array.isArray(names)?names:[names]){
    const escaped=name.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),pattern=new RegExp(`\\benum\\s+(?:(class|struct)\\s+)?${escaped}(?:\\s*:\\s*[^\\{]+)?\\s*\\{`,"g");
    for(const match of String(source??"").matchAll(pattern)){
      if(!isCodePosition(source,match.index)||!isNamespaceScopeAt(source,match.index))continue;
      const namespaces=namespaceStackAt(source,match.index);
      if(namespaces.length!==targetNamespace.length||!namespaces.every((namespace,index)=>namespace===targetNamespace[index]))continue;
      const open=source.indexOf("{",match.index),close=matchingBrace(source,open);
      if(close<0)continue;
      let end=close+1;while(/[ \t]/.test(source[end]??""))end++;if(source[end]===";")end++;
      return{info:{...parsedEnumInfo(name,source.slice(open+1,close)),scoped:Boolean(match[1])},declaration:source.slice(match.index,end)}
    }
  }
  return null
}
export function enumInfoByTerminal(source,terminal){
  ...
  const value=String(source??""),pattern=/\benum(?:\s+(?:class|struct))?(?:\s+([A-Za-z_]\w*))?[^;{}]*\{/g;
  for(const match of value.matchAll(pattern)){
    const open=value.indexOf("{",match.index),close=matchingBrace(value,open);
    if(close<0)continue;
    const info=parsedEnumInfo(match[1]??`__anonymous_${terminal}`,value.slice(open+1,close)),last=[...info.identifiers].reverse().find(identifier=>typeof identifier==="string");
    if(last===terminal)return info;
  }
  return null
}
function enumInfoByQualifiedAlias(source,qualified){const reference=/\b([A-Za-z_]\w*)::([A-Za-z_]\w*)\b/.exec(String(qualified??""));if(!reference)return null;const [,alias,terminal]=reference,usingType=new RegExp(`\\busing\\s+${alias}\\s*=\\s*([^;]+)\\s*;`).exec(source||"")?.[1]?.trim(),type=usingType??alias,name=baseTypeName(type),body=plainStructBody(source,name);if(body===null)return null;const match=/\benum(?:\s+(?:class\s+)?[A-Za-z_]\w*)?\s*\{([\s\S]*?)\};/.exec(body);if(!match)return null;const info=parsedEnumInfo(name,match[1]),last=[...info.identifiers].reverse().find(value=>typeof value==="string");return last===terminal?info:null}
...
function outOfLineDefaultedDefinitions(source,className,preserveNamespace=false,fallbackNamespaces=[]){
  const name=baseTypeName(className).split("::").at(-1),escaped=name.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),
    pattern=new RegExp(`^[ \\t]*(?:template\\s*<[^;{}]+>\\s*)?(?:${escaped}(?:\\s*<[^;{}]+>)?\\s*::\\s*)?${escaped}(?:\\s*<[^;{}]+>)?\\s*::\\s*~?${escaped}\\s*\\([^;{}]*\\)\\s*(?:noexcept\\s*)?=\\s*(?:default|delete)\\s*;`,"gm"),found=[];
  for(const match of source.matchAll(pattern)){
    if(!isCodePosition(source,match.index)||!isNamespaceScopeAt(source,match.index))continue;
    let definition=match[0].trim();
    if(preserveNamespace){
      const namespaces=namespaceStackAt(source,match.index),effectiveNamespaces=namespaces.length?namespaces:fallbackNamespaces;
      if(effectiveNamespaces.length)definition=`${effectiveNamespaces.map(namespace=>`namespace ${namespace} {`).join("\n")}\n${definition}\n${effectiveNamespaces.map(()=>"}").join("\n")}`
    }
    found.push(definition)
  }
  return[...new Set(found)]
}
function outOfLineDefinitions(source,className,preserveNamespace=false,fallbackNamespaces=[]){
  const name=baseTypeName(className).split("::").at(-1),escaped=name.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),method=`(?:~?${escaped}|operator\\s*(?:\\[\\]|\\(\\)|[+*/%<>=!&|^~-]+)|[A-Za-z_]\\w*)`,pattern=new RegExp(`\\b${escaped}(?:\\s*<[^;{}]+>)?\\s*::\\s*(?:[A-Za-z_]\\w*(?:\\s*<[^;{}]+>)?\\s*::\\s*)*${method}\\s*\\(`,"g"),found=[];
  for(const match of source.matchAll(pattern)){
    if(!isCodePosition(source,match.index))continue;
    const lineStart=source.lastIndexOf("\n",match.index)+1,prefix=source.slice(lineStart,match.index);
    let open=source.indexOf("{",match.index);
    if(/[=(.]/.test(prefix)||open<0||source.slice(match.index,open).includes(";"))continue;
    for(let attempts=0;attempts<16;attempts++){
      const candidateClose=matchingBrace(source,open);
      if(candidateClose<0)break;
      let cursor=candidateClose+1;
      while(/\s/.test(source[cursor]??""))cursor++;
      if(source[cursor]!=="{"&&source[cursor]!==",")break;
      open=source.indexOf("{",candidateClose+1);
      if(open<0)break
    }
    const close=matchingBrace(source,open);
    if(close<0)continue;
    const leading=source.slice(0,lineStart),
      detachedReturn=/(?:^|\n)([ \t]*(?:(?:const|static|inline|constexpr|virtual)\s+)*(?:[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*(?:\s*<[^;{}\n]+>)?)(?:\s*[*&])?[ \t]*\r?\n[ \t]*)$/.exec(leading);
    let definitionStart=detachedReturn?leading.length-detachedReturn[1].length:lineStart;
    const beforeDefinition=source.slice(0,definitionStart),
      templatePrefix=/(?:^|\n)([ \t]*template\s*<[^>{}]*>\s*\r?\n[ \t]*)$/.exec(beforeDefinition);
    if(templatePrefix)definitionStart=beforeDefinition.length-templatePrefix[1].length;
    let definition=source.slice(definitionStart,close+1).trim();
    const signaturePrefix=source.slice(lineStart,open).replace(/\/\*[\s\S]*?\*\//g,"").replace(/\/\/.*$/gm,"").trim();
    if(!/^(?:\[\[[^\]]+\]\]\s*)*[A-Za-z_~][\w:<>,*&\s~+\-/%=\[\]!|^]*\([^;{}]*\)(?:\s+(?:const|noexcept|override|final))*(?:\s*->\s*[^;{}]+)?\s*(?::[\s\S]*)?$/.test(signaturePrefix))continue;
    if(preserveNamespace){
      const namespaces=namespaceStackAt(source,match.index),effectiveNamespaces=namespaces.length?namespaces:fallbackNamespaces;
      if(effectiveNamespaces.length)definition=`${effectiveNamespaces.map(namespace=>`namespace ${namespace} {`).join("\n")}\n${definition}\n${effectiveNamespaces.map(()=>"}").join("\n")}`
    }
  ...
  return [...new Set([...found,...outOfLineDefaultedDefinitions(source,className,preserveNamespace,fallbackNamespaces)])]
}
function outOfLineFreeFunctionDefinitions(source,functionName,preserveNamespace=false,fallbackNamespaces=[]){const name=baseTypeName(functionName).split("::").at(-1),escaped=name.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),pattern=new RegExp(`^[ \\t]*(?:template\\s*<[^>]*>\\s*)?(?:(?:inline|static|constexpr|const|volatile|unsigned|signed|long|short)\\s+)*(?:[A-Za-z_]\\w*(?:::[A-Za-z_]\\w*)*(?:\\s*<[^;{}]+>)?[&*\\s]+)+(?:[A-Za-z_]\\w*::)*${escaped}(?:\\s*<[^;{}]+>)?\\s*\\(`,"gm"),found=[];for(const match of source.matchAll(pattern)){if(!isCodePosition(source,match.index)||!isNamespaceScopeAt(source,match.index))continue;const open=source.indexOf("{",match.index);if(open<0||source.slice(match.index,open).includes(";"))continue;const close=matchingBrace(source,open);if(close<0)continue;let definition=source.slice(match.index,close+1).trim();if(preserveNamespace&&!new RegExp(`\\b[A-Za-z_]\\w*::${escaped}(?:\\s*<[^;{}]+>)?\\s*\\(`).test(definition)){const namespaces=namespaceStackAt(source,match.index),effectiveNamespaces=namespaces.length?namespaces:fallbackNamespaces;if(effectiveNamespaces.length)definition=`${effectiveNamespaces.map(namespace=>`namespace ${namespace} {`).join("\n")}\n${definition}\n${effectiveNamespaces.map(()=>"}").join("\n")}`}found.push(definition)}return[...new Set(found)]}
function outOfLineStaticDefinitions(source,className,preserveNamespace=false,fallbackNamespaces=[]){const name=baseTypeName(className).split("::").at(-1),escaped=name.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),pattern=new RegExp(`^[ \\t]*(?:template\\s*<[^;{}]+>\\s*)?(?:(?:constexpr|const|volatile|inline|static|unsigned|signed|long|short)\\s+)*(?:[A-Za-z_]\\w*(?:::[A-Za-z_]\\w*)*(?:\\s*<[^;{}]+>)?)[*&\\s]+${escaped}(?:\\s*<[^;{}]+>)?::(?!operator\\b)[A-Za-z_]\\w*(?:\\s*\\[[^\\]]*\\])*(?=\\s*(?:\\{|=|;))`,"gm"),found=[];for(const match of source.matchAll(pattern)){if(!isCodePosition(source,match.index)||!isNamespaceScopeAt(source,match.index))continue;const end=namespaceStatementEnd(source,match.index+match[0].length);if(end<0)continue;let definition=source.slice(match.index,end).trim();if(!definition)continue;if(preserveNamespace){const namespaces=namespaceStackAt(source,match.index),effectiveNamespaces=namespaces.length?namespaces:fallbackNamespaces;if(effectiveNamespaces.length)definition=`${effectiveNamespaces.map(namespace=>`namespace ${namespace} {`).join("\n")}\n${definition}\n${effectiveNamespaces.map(()=>"}").join("\n")}`}found.push(definition)}return[...new Set(found)]}
...
function namespaceGlobalDefinitions(source,reference,relativeNamespace=[],existing=""){const candidates=[],pattern=/(?:^|(?<=;))[ \t]*(?!(?:struct|class|enum|union|namespace|using|typedef|template)\b)(?:(?:extern|static|inline|constexpr|const|volatile|unsigned|signed|long|short)\s+)*(?:[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*(?:\s*<[^;{}]+>)?)(?:\s*[*&]\s*|\s+)([A-Za-z_]\w*)\s*(?:\[[^\]]*\]\s*)*(?=\s*(?:=|\{|;))/gm;for(const match of source.matchAll(pattern)){if(!isCodePosition(source,match.index)||!isNamespaceScopeAt(source,match.index))continue;const end=namespaceStatementEnd(source,match.index+match[0].length);if(end<0)continue;let definition=browserModelIdentityDefinition(source.slice(match.index,end).trim());if(/^extern\b/.test(definition)&&!/[\{=]/.test(definition))continue;const namespaces=namespaceStackAt(source,match.index),relative=relativeNamespace.length<=namespaces.length&&relativeNamespace.every((name,index)=>namespaces[index]===name)?namespaces.slice(relativeNamespace.length):namespaces;if(relative.length)definition=`${relative.map(namespace=>`namespace ${namespace} {`).join("\n")}\n${definition}\n${relative.map(()=>"}").join("\n")}`;candidates.push({name:match[1],definition})}const selected=new Set;let expanded=String(reference??"");for(let pass=0;pass<candidates.length;pass++){let changed=false;for(const candidate of candidates){if(selected.has(candidate)||!new RegExp(`\\b${candidate.name}\\b`).test(expanded))continue;selected.add(candidate);expanded+=`\n${candidate.definition}`;changed=true}if(!changed)break}return[...new Set(candidates.filter(candidate=>selected.has(candidate)&&!String(existing).includes(candidate.definition)).map(candidate=>candidate.definition))].join("\n")}
function referencedExternGlobalDefinitions(sourceFiles,reference,existing=""){
  const referenceSource=String(reference??""),
    names=[...new Set([...referenceSource.matchAll(/^\s*extern\s+(?:(?:const|volatile|unsigned|signed|long|short)\s+)*(?:[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*(?:\s*<[^;{}\n]+>)?)(?:\s*[*&]\s*|\s+)([A-Za-z_]\w*)\s*(?:\[[^\]\n]*\]\s*)*;/gm)].filter(match=>isCodePosition(referenceSource,match.index)).map(match=>match[1]))];
...
function exactNamespaceGlobalDefinitions(source,reference,targetNamespace=[]){
  const candidates=[],pattern=/^[ \t]*(?!(?:extern|struct|class|enum|union|namespace|using|typedef|template)\b)(?:(?:static|inline|constexpr|const|volatile|unsigned|signed|long|short)\s+)*(?:[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*(?:\s*<[^;{}]+>)?)(?:\s*[*&]\s*|\s+)([A-Za-z_]\w*)\s*(?:\[[^\]]*\]\s*)*(?=\s*(?:=|\{|;))/gm;
  for(const match of source.matchAll(pattern)){
    if(!isCodePosition(source,match.index)||!isNamespaceScopeAt(source,match.index))continue;
    const namespaces=namespaceStackAt(source,match.index);
    if(namespaces.length!==targetNamespace.length||!namespaces.every((name,index)=>name===targetNamespace[index]))continue;
    const end=namespaceStatementEnd(source,match.index+match[0].length);
    if(end>=0)candidates.push({name:match[1],definition:browserModelIdentityDefinition(source.slice(match.index,end).trim())});
  }
...
function removeOutOfLineDefinitions(source,className){const name=baseTypeName(className).split("::").at(-1),escaped=name.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),method=`(?:~?${escaped}|operator\\s*(?:\\[\\]|\\(\\)|[+*/%<>=!&|^~-]+)|[A-Za-z_]\\w*)`,ranges=[];for(const match of source.matchAll(new RegExp(`\\b${escaped}(?:\\s*<[^;{}]+>)?::(?:[A-Za-z_]\\w*(?:\\s*<[^;{}]+>)?::)*${method}\\s*\\(`,"g"))){const open=source.indexOf("{",match.index);if(open<0||source.slice(match.index,open).includes(";"))continue;const close=matchingBrace(source,open);if(close>=0)ranges.push([source.lastIndexOf("\n",match.index)+1,close+1])}let result=source;for(const [start,end] of ranges.reverse())result=result.slice(0,start)+result.slice(end);return result}
...
function implementationSupportDeclarations(source,implementation){
  const found=[],supportFunctions=[],known=new Set;let reference=String(implementation??"");
  for(let depth=0;depth<16;depth++){
    let changed=false;
    for(const pattern of [/^\s*typedef\s+[^;]+\s+([A-Za-z_]\w*)\s*;/gm,/^\s*using\s+([A-Za-z_]\w*)\s*=\s*[^;]+;/gm])for(const match of source.matchAll(pattern)){const definition=match[0].trim();if(known.has(definition)||!new RegExp(`\\b${match[1]}\\b`).test(reference))continue;known.add(definition);found.push(definition);reference+=`\n${definition}`;changed=true}
    for(const match of source.matchAll(/^[ \t]*(?:template\s*<[^>]*>\s*)?(?:(?:static|inline|constexpr)[ \t]+)*(?:[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*(?:[ \t]*<[^;{}\n]+>)?[&* \t]+)+([A-Za-z_]\w*)[ \t]*\([^;{}\n]*\)[ \t]*\{/gm)){if(/::\s*[A-Za-z_]\w*\s*\(/.test(match[0])||!isNamespaceScopeAt(source,match.index)||!new RegExp(`\\b${match[1]}(?:\\s*<[^;{}]+>)?\\s*\\(`).test(reference))continue;const open=source.indexOf("{",match.index),close=matchingBrace(source,open),rawDefinition=close>=0?source.slice(match.index,close+1).trim():"",namespaces=namespaceStackAt(source,match.index),definition=rawDefinition&&namespaces.length?`${namespaces.map(namespace=>`namespace ${namespace} {`).join("\n")}\n${rawDefinition}\n${namespaces.map(()=>"}").join("\n")}`:rawDefinition;if(!definition||known.has(definition)||implementation.includes(definition)||implementation.includes(rawDefinition))continue;known.add(definition);found.push(definition);supportFunctions.push(definition);reference+=`\n${definition}`;changed=true}
    if(!changed)break;
  }
  const usingDeclarations=found.length?[...source.matchAll(/^\s*using\s+namespace\s+[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*\s*;/gm)].map(match=>match[0].trim()):[],functionDeclarations=supportFunctions.map(freeFunctionDeclaration);return[...new Set([...usingDeclarations,...functionDeclarations,...found].filter(Boolean))].join("\n")
}
function referencedLocalFreeFunctionDefinitions(source,reference){
  const candidates=new Map;
  for(const match of source.matchAll(/^[ \t]*(?:template\s*<[^>]*>\s*)?(?:(?:inline|static|constexpr|consteval|const|volatile|unsigned|signed|long|short)\s+)*(?:[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*(?:\s*<[^;{}]+>)?[&*\s]+)+([A-Za-z_]\w*)\s*\([^;{}]*\)\s*(?:const\s*)?(?:noexcept\s*)?\{/gm)){
    if(/::\s*[A-Za-z_]\w*\s*\(/.test(match[0])||!isCodePosition(source,match.index)||!isNamespaceScopeAt(source,match.index))continue;
    const open=source.indexOf("{",match.index),close=matchingBrace(source,open);
    if(close<0)continue;
    let definition=source.slice(match.index,close+1).trim();
    const name=match[1],namespaces=namespaceStackAt(source,match.index);
    if(namespaces.length)definition=`${namespaces.map(namespace=>`namespace ${namespace} {`).join("\n")}\n${definition}\n${namespaces.map(()=>"}").join("\n")}`;
    if(hasRackUiReference(definition)||/\bNVG(?:color|context)\b|\bnvg[A-Z]\w*\s*\(/.test(definition)||/\bAPP\s*->\s*(?!engine\b)/.test(definition))continue;
    if(!candidates.has(name))candidates.set(name,[]);
    candidates.get(name).push(definition)
  }
  const selected=[],seen=new Set;let expanded=reference;
  for(let pass=0;pass<candidates.size;pass++){
    let changed=false;
    for(const [name,definitions] of candidates){
      const referenced=new RegExp(`\\b${name}(?:\\s*<[^;{}]+>)?\\s*(?:\\(|(?=[,);}\\]]))`).test(sourceWithoutCommentsAndLiterals(expanded));
      if(seen.has(name)||!referenced||outOfLineFreeFunctionDefinitions(expanded,name).length)continue;
      seen.add(name);changed=true;
      for(const definition of definitions){selected.push(definition);expanded+=`\n${definition}`}
    }
    if(!changed)break
  }
  return[...new Set(selected)]
}
function freeFunctionDeclaration(definition){
  const pattern=/^[ \t]*(?:template\s*<[^>]*>\s*)?(?:(?:inline|static|constexpr|consteval|const|volatile|unsigned|signed|long|short)\s+)*(?:[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*(?:\s*<[^;{}]+>)?[&*\s]+)+[A-Za-z_]\w*\s*\([^;{}]*\)\s*(?:const\s*)?(?:noexcept\s*)?\{/gm,match=pattern.exec(definition);
  if(!match)return"";
  const open=definition.indexOf("{",match.index),declaration=`${definition.slice(match.index,open).trim()};`,namespaces=namespaceStackAt(definition,match.index);
  return namespaces.length?`${namespaces.map(namespace=>`namespace ${namespace} {`).join("\n")}\n${declaration}\n${namespaces.map(()=>"}").join("\n")}`:declaration
}
...
function dedupeFreeFunctionDefinitions(source){const pattern=/^[ \t]*(?:(?:inline|static|constexpr|consteval|const|volatile|unsigned|signed|long|short)\s+)*(?:[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*(?:\s*<[^;{}]+>)?[&*\s]+)+([A-Za-z_]\w*)\s*\([^;{}]*\)\s*(?:const\s*)?(?:noexcept\s*)?\{/gm,seen=new Set,ranges=[];for(const match of source.matchAll(pattern)){if(!isCodePosition(source,match.index)||!isNamespaceScopeAt(source,match.index))continue;const open=source.indexOf("{",match.index),close=matchingBrace(source,open);if(close<0)continue;const definition=source.slice(match.index,close+1).trim();if(seen.has(definition))ranges.push([match.index,close+1]);else seen.add(definition)}let result=source;for(const [start,end] of ranges.reverse())result=result.slice(0,start)+result.slice(end);return result}
function classImplementations(sourceFiles,preferredFile,className){
  const collect=source=>{
    let definitions=[...outOfLineStaticDefinitions(source,className),...outOfLineDefinitions(source,className)];
...
function preludeTypeImplementations(sourceFiles,prelude,reference,excludedType,preferredFile=null){
  ...
  for(let pass=0;pass<candidates.length*2;pass++){
    ...
    for(const name of candidates){
      ...
      const definitionsByFile=sourceFiles.map(file=>{const source=sourceForFile(file),hasScopedDefinition=source.includes(`${name}::`)||source.includes(`${name}<`);return{file,definitions:hasScopedDefinition?outOfLineDefinitions(source,name,true).filter(definition=>!rackUiPattern.test(definition.replace(/\b(?:rack::math::|math::)?Vec\b/g,""))):[]}}),preferredDefinitions=preferredFile?definitionsByFile.find(item=>path.resolve(item.file)===path.resolve(preferredFile)&&item.definitions.length):null,implementationDefinitions=preferredDefinitions?[preferredDefinitions]:definitionsByFile;
  ...
  const directories=new Set([...implementationFiles].map(file=>path.dirname(file))),supportFiles=sourceFiles.filter(file=>/\.(?:c|cc|cpp|cxx)$/.test(file)&&directories.has(path.dirname(file))),support=[],known=new Set;let supportReference=implementations.join("\n");for(let pass=0;pass<16;pass++){let changed=false;for(const file of supportFiles){const source=sourceForFile(file);for(const definition of referencedLocalFreeFunctionDefinitions(source,supportReference)){if(known.has(definition))continue;known.add(definition);support.push({source,definition});supportReference+=`\n${definition}`;changed=true}}if(!changed)break}const declarations=support.map(({definition})=>freeFunctionDeclaration(definition)).filter(Boolean),globals=[...new Set(support.map(({source})=>namespaceGlobalDefinitions(source,support.map(item=>item.definition).join("\n"))).filter(Boolean))];return[...new Set(declarations),...globals,...support.map(item=>item.definition),...implementations]
}
...
function calls(body,name,constants={}){const source=body||"",result=[],pattern=new RegExp(`\\b${name}(?:\\s*<[^;(){}]+>)?\\s*\\(`,"g");for(const match of source.matchAll(pattern)){const open=source.indexOf("(",match.index),close=matchingParenthesis(source,open);if(close<0)continue;const value=source.slice(open+1,close).replace(/\s+/g," ").trim(),prefix=source.slice(0,match.index),loops=[];for(const loop of prefix.matchAll(/for\s*\(\s*(?:int|long|auto|unsigned(?:\s+(?:int|long))?|(?:std::)?size_t)\s+([A-Za-z_]\w*)\s*=\s*([^;]+)\s*;\s*\1\s*<\s*([^;]+)\s*;[^)]*\)\s*\{/g)){const brace=source.indexOf("{",loop.index),end=matchingBrace(source,brace);if(brace<match.index&&end>match.index)loops.push({...loop,brace})}let contexts=[{}];for(const loop of loops){const expanded=[];for(const context of contexts){const scoped={...constants,...context},start=numberLiteral(loop[2],Number.NaN,scoped),end=numberLiteral(loop[3],Number.NaN,scoped);if(!Number.isSafeInteger(start)||!Number.isSafeInteger(end)||end<=start||end-start>256)continue;for(let index=start;index<end;index++)expanded.push({...context,[loop[1]]:index});if(expanded.length>2048)break}contexts=expanded;if(!contexts.length||contexts.length>2048)break}if(contexts.length)for(const context of contexts){let expanded=value;for(const [identifier,replacement] of Object.entries(context))expanded=replaceIdentifierOutsideStrings(expanded,identifier,replacement);const loopStart=loops.at(-1)?.brace+1??Math.max(0,match.index-2000),localSource=source.slice(loopStart,match.index);for(const binding of localSource.matchAll(/\b(?:auto|(?:std::)?string)\s+([A-Za-z_]\w*)\s*=\s*([^;]+)\s*;/g)){let expression=binding[2];for(const [identifier,replacement] of Object.entries(context))expression=replaceIdentifierOutsideStrings(expression,identifier,replacement);const resolved=stringLiteral(expression,null,{...constants,...context});if(typeof resolved==="string")expanded=replaceIdentifierOutsideStrings(expanded,binding[1],JSON.stringify(resolved))}result.push(expanded)}else result.push(value)}return result}
function paramSnapCalls(body,constants={}){
  const source=sourceWithoutComments(body||"")
    .replace(
      /\bparamQuantities\s*\[\s*([^\]]+?)\s*\]\s*->\s*snapEnabled\s*=\s*true\s*;/g,
      (_match,expression)=>`rackWebSnapParam(${expression});`,
    )
    .replace(
      /\b(?:[A-Za-z_]\w*\s*->\s*)?getParamQuantity\s*\(\s*([^)]+?)\s*\)\s*->\s*snapEnabled\s*=\s*true\s*;/g,
      (_match,expression)=>`rackWebSnapParam(${expression});`,
    );
  return calls(source,"rackWebSnapParam",constants);
}
function menuParamCalls(body,constants={}){return calls(body,"configMenuParam",constants).map(call=>{const values=splitArguments(call);if(values.length>=5)return values.slice(0,5).join(", ");if(values.length<3)return call;const options=/^\s*\{([\s\S]*)\}\s*$/.exec(values[3]??""),maximum=options?Math.max(0,splitArguments(options[1]).length-1):1;return`${values[0]}, 0, ${maximum}, ${values[1]}, ${values[2]}`})}
...
function scalarJsonStateKeys(source,constants={}){
  const stringDefines=new Map([...source.matchAll(/^\s*#define\s+([A-Za-z_]\w*)\s+"([^"]*)"\s*$/gm)].map(match=>[match[1],match[2]]));for(const [name,value] of stringDefines)source=source.replace(new RegExp(`\\b${name}\\b`,"g"),JSON.stringify(value));
...
function surgeVcoSpecializations(sourceFiles,moduleClass){const type=/\bVCO\s*<\s*([^>]+)>/.exec(moduleClass)?.[1]?.trim();if(!type)return"";const escaped=type.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),found=[],methods=new Set;for(const file of sourceFiles){const source=fs.readFileSync(file,"utf8"),pattern=new RegExp(`template\\s*<>\\s*[^;{}]+\\bVCOConfig\\s*<\\s*${escaped}\\s*>\\s*::\\s*([A-Za-z_]\\w*)\\s*\\(`,"g");for(const match of source.matchAll(pattern)){if(["getLayout","addMenuItems"].includes(match[1]))continue;const open=source.indexOf("{",match.index),semicolon=source.indexOf(";",match.index);if(open<0||(semicolon>=0&&semicolon<open))continue;const close=matchingBrace(source,open);if(close<0)continue;methods.add(match[1]);found.push(source.slice(match.index,close+1).trim())}}if(!methods.has("configureVCOSpecificParameters"))for(const file of sourceFiles){const source=fs.readFileSync(file,"utf8"),match=/template\s*<\s*int\s+oscType\s*>\s*inline\s+void\s+VCOConfig\s*<\s*oscType\s*>\s*::\s*configureVCOSpecificParameters\s*\(/.exec(source);if(!match)continue;const open=source.indexOf("{",match.index),close=matchingBrace(source,open);if(open>=0&&close>=0)found.push(source.slice(match.index,close+1).trim());break}return[...new Set(found)].join("\n\n")}
...
function adaptLeviathanUndertowBrowserImplementation(source){
  ...
  source=source
    .replace(/^\s*bool\s+false\s*;\s*$/gm,"")
    .replace(/^\s*bool\s+false\s*\{[^{}]*\}\s*$/gm,"");
  const moduleMatch=classDeclarationMatch(source,"Undertow");
  if(moduleMatch){
    const open=source.indexOf("{",moduleMatch.index),close=open>=0?matchingBrace(source,open):-1;
    if(close>=0){
      const body=source.slice(open+1,close);
      if(!body.includes("rackWebPreview"))source=source.slice(0,open+1)+adaptLeviathanUndertowBrowserBody(body)+source.slice(close);
    }
  }
...
function adapterSource(target,manifest,license,definitionFile,registrationFile,registration,prelude,body,implementations,inherited,detected,compileEligible,sourceDir,vcoSpecializations=""){
  ...
  if(directDmaHeader&&/\bDMA(?:Expander|Host)Module\s*</.test(`${detected.inheritance?.directBase??""}\n${body}`)){
    const directIncludes=new Set(includes(fs.readFileSync(directDmaHeader,"utf8")).map(file=>path.basename(file))),includedHeaders=(detected.sourceFiles??[]).filter(file=>directIncludes.has(path.basename(file)));
...
function stripEmbeddedResourceDocumentation(source){
  ...
  const replacements=[],pattern=/\bstd::string\s+([A-Za-z_]\w*(?:::[A-Za-z_]\w*)*)::([A-Za-z_]\w*)\s*\([^;{}]*\)\s*\{/g;
  for(const match of source.matchAll(pattern)){
    const open=source.indexOf("{",match.index),close=matchingBrace(source,open);
    if(close<0||!/\bcmrc::[A-Za-z_]\w*::get_filesystem\s*\(/.test(source.slice(open+1,close)))continue;
    replacements.push([open,close+1,"{ return \"\"; }"]);
  }
...
function airwinBrowserSuite(output,sourceDir,source){
  ...
  const includes=[...fs.readFileSync(moduleAdd,"utf8").matchAll(/^\s*#include\s+"([^"]+)"\s*$/gm)].map(match=>`#include ${JSON.stringify(path.resolve(path.dirname(moduleAdd),match[1]))}`);
  ...
  const implementationIncludes=implementations.map(implementation=>{const source=fs.readFileSync(implementation,"utf8"),guard=/^\s*#ifndef\s+([A-Za-z_]\w*)\s*\n\s*#include\s+"[^"]+"\s*\n\s*#endif/m.exec(source)?.[1];return guard?[`#define ${guard} 1`,`#include ${JSON.stringify(implementation)}`,`#undef ${guard}`].join("\n"):`#include ${JSON.stringify(implementation)}`});
...
function stubInlineVoidMethod(source,name){const escaped=name.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),match=new RegExp(`\\bvoid\\s+${escaped}\\s*\\([^)]*\\)(?:\\s+override)?\\s*\\{`).exec(source);if(!match)return source;const open=source.indexOf("{",match.index),close=matchingBrace(source,open);return close<0?source:source.slice(0,open+1)+source.slice(close)}
function stripSurgeRackCustomEditor(source){const match=/template\s*<>\s*rack::Widget\s*\*\s*VCOConfig\s*<[^>]+>\s*::\s*createCustomEditorAt\s*\(/.exec(source);if(!match)return source;const open=source.indexOf("{",match.index),close=matchingBrace(source,open);return open<0||close<0?source:source.slice(0,match.index)+source.slice(close+1)}
...
function makefileCompileDefinitions(sourceDir){
  const file=path.join(sourceDir,"Makefile");if(!fs.existsSync(file))return[];
  const source=fs.readFileSync(file,"utf8").replace(/\\\r?\n/g," "),definitions=[];let conditionalDepth=0;
  for(const line of source.split(/\r?\n/)){
    if(/^\s*(?:ifdef|ifndef|ifeq|ifneq)\b/.test(line)){conditionalDepth++;continue}
    if(/^\s*endif\b/.test(line)){conditionalDepth=Math.max(0,conditionalDepth-1);continue}
    if(conditionalDepth)continue;
    for(const match of line.matchAll(/(?:^|\s)-D([A-Za-z_]\w*)(?:=([^\s#]+))?/g))definitions.push(`-D${match[1]}${match[2]===undefined?"":`=${match[2]}`}`)
  }
  return[...new Set(definitions)]
}
function makefileIncludeDirectories(sourceDir){const file=path.join(sourceDir,"Makefile");if(!fs.existsSync(file))return[];const source=fs.readFileSync(file,"utf8").replace(/\\\r?\n/g," "),root=path.resolve(sourceDir),prefix=`${root}${path.sep}`,directories=[];for(const match of source.matchAll(/(?:^|\s)-I(?:"([^"]+)"|'([^']+)'|([^\s#]+))/g)){const token=match[1]??match[2]??match[3];if(!token||token.includes("$"))continue;const directory=path.resolve(root,token);if((directory===root||directory.startsWith(prefix))&&fs.existsSync(directory)&&fs.statSync(directory).isDirectory())directories.push(directory)}return[...new Set(directories)]}
function makefileImplementationSources(sourceDir){
  const file=path.join(sourceDir,"Makefile");if(!fs.existsSync(file))return[];
  const source=fs.readFileSync(file,"utf8").replace(/\\\r?\n/g," "),variables=new Map;
  for(const line of source.split(/\r?\n/)){const assignment=/^\s*([A-Za-z_]\w*)\s*(?::|\?)?=\s*([^#]*)/.exec(line);if(assignment)variables.set(assignment[1],assignment[2].trim())}
  const expand=value=>{let result=value;for(let pass=0;pass<8;pass++){const next=result.replace(/\$\(([A-Za-z_]\w*)\)/g,(match,name)=>variables.get(name)??match);if(next===result)break;result=next}return result},resolved=[];
  for(const line of source.split(/\r?\n/)){
    const assignment=/^\s*SOURCES\s*\+?=\s*([^#]*)/.exec(line);if(!assignment)continue;
    for(const token of expand(assignment[1]).trim().split(/\s+/)){if(!/\.(?:c|cc|cpp|cxx)$/.test(token)||token.includes("$"))continue;const file=path.resolve(sourceDir,token);if(fs.existsSync(file)&&fs.statSync(file).isFile())resolved.push(file)}
  }
  return[...new Set(resolved)]
}
function chuckEmscriptenImplementationSources(sourceDir){
  const chuckSourceRoot=path.join(sourceDir,"chuck","src"),file=path.join(chuckSourceRoot,"makefile");if(!fs.existsSync(file))return[];
  const source=fs.readFileSync(file,"utf8").replace(/\\\r?\n/g," "),assignment=/^\s*EMSCRIPTENSRCS\s*=\s*([^#]*)/m.exec(source);if(!assignment)return[];
  return[...new Set(assignment[1].trim().split(/\s+/).filter(token=>/\.(?:c|cc|cpp|cxx)$/.test(token)).map(token=>path.resolve(chuckSourceRoot,token)).filter(file=>fs.existsSync(file)&&fs.statSync(file).isFile()))]
}
...
function cmakeCompileDefinitions(sourceDir){const file=path.join(sourceDir,"CMakeLists.txt");if(!fs.existsSync(file))return[];const source=fs.readFileSync(file,"utf8"),definitions=[];for(const match of source.matchAll(/\bset\s*\(\s*([A-Z][A-Z0-9_]+)\s+([0-9]+|TRUE|FALSE)\s+CACHE\s+(?:STRING|BOOL)\b/g)){const value=match[2]==="TRUE"?"1":match[2]==="FALSE"?"0":match[2];definitions.push(`-D${match[1]}=${value}`)}if(fs.existsSync(path.join(sourceDir,"surge/libs/libsamplerate/src/src_sinc.c")))definitions.push("-DENABLE_SINC_FAST_CONVERTER=1");return[...new Set(definitions)]}
...
function execCompiler(command,args){
  if(process.env.RACK_WEB_TRACE_COMPILER)
    process.stderr.write(`${command} ${args.map(argument=>JSON.stringify(argument)).join(" ")}\n`);
  try{return execFileSync(command,args,{stdio:["inherit","inherit","pipe"]})}
  catch(error){const stderr=error?.stderr?.toString?.()??"";throw new Error(`${error instanceof Error?error.message:`${command} failed`}${stderr?`\n${stderr}`:""}`)}
}
...
void rdft(int, int, double*, int*, double*);
  ...
  const allSourceHeaders=files(sourceDir).filter(file=>/\.(?:h|hh|hpp)$/.test(file)&&!/(?:^|[\\/])(?:metamodule-)?plugin-libc(?:[\\/]|$)/.test(path.relative(sourceDir,file))),unresolvedIncludeTokens=[...new Set(sourceFiles.flatMap(file=>includes(fs.readFileSync(file,"utf8"))).filter(token=>token.includes("/")||/^lib[A-Z]/.test(token)))],inferredHeaderRootArgs=[...new Set(unresolvedIncludeTokens.flatMap(token=>{const normalized=token.split("/").join(path.sep),suffix=`${path.sep}${normalized}`,candidate=selectIncludeCandidate(allSourceHeaders.filter(file=>file.endsWith(suffix)),sourceDir);return candidate?[`-idirafter${candidate.slice(0,-normalized.length).replace(/[\\\/]$/,"")}`]:[]}))],implementationIncludeFiles=[...new Set([...sourceFiles,...externalSources])],quotedImplementationTokens=[...new Set(implementationIncludeFiles.flatMap(file=>[...fs.readFileSync(file,"utf8").matchAll(/^\s*#include\s+"([^"]+)"/gm)].map(match=>match[1])))],inferredQuoteRootArgs=[...new Set(quotedImplementationTokens.flatMap(token=>{const normalized=token.split("/").join(path.sep),suffix=`${path.sep}${normalized}`,candidate=selectIncludeCandidate(allSourceHeaders.filter(file=>file.endsWith(suffix)),sourceDir);return candidate?[`-iquote${candidate.slice(0,-normalized.length).replace(/[\\\/]$/,"")}`]:[]}))],quotePriority=argument=>{const directory=argument.slice("-iquote".length);return directory.startsWith(`${path.join(sourceDir,"surge","src")}${path.sep}`)?0:directory.startsWith(`${path.join(sourceDir,"src")}${path.sep}`)?1:directory.startsWith(`${sourceDir}${path.sep}`)?2:3},prioritizedIncludeArgs=[...new Set([...includeArgs.filter(argument=>!argument.startsWith("-iquote")),...(browserHeaderRoot?[`-iquote${browserHeaderRoot}`]:[]),...inferredHeaderRootArgs,...includeArgs.filter(argument=>argument.startsWith("-iquote")).sort((left,right)=>quotePriority(left)-quotePriority(right)||left.localeCompare(right)),...inferredQuoteRootArgs])];
  const definitions=source=>[...source.matchAll(/\b([A-Za-z_]\w*)::(~?[A-Za-z_]\w*)\s*\(/g)].map(match=>`${match[1]}::${match[2]}`),headerDefinitions=new Set(sourceFiles.filter(file=>/\.(?:hpp|hh|h)$/.test(file)).flatMap(file=>definitions(fs.readFileSync(file,"utf8")))),hasPlatformOverrides=externalSources.some(file=>definitions(fs.readFileSync(file,"utf8")).some(symbol=>headerDefinitions.has(symbol))),linkerArgs=hasPlatformOverrides?["-Wl,--allow-multiple-definition"]:[],
  ...
  try {
    ...
    const longjmpArgs=usesBrowserLua?["-s","SUPPORT_LONGJMP=wasm"]:[];
    const pffftSimdArgs=externalPffft?["-DPFFFT_ENABLE_NEON=1","-msimd128"]:[];
    const objects=cSources.map((source,index)=>{const object=path.join(temporaryObjects,`${index}-${path.basename(source,".c")}.o`);execCompiler("emcc",[source,...prioritizedIncludeArgs,"-std=gnu11",...optimizationFlags,"-DNDEBUG","-DTEST",...compileDefinitions,...cPlatformDefinitions,...pffftSimdArgs,...longjmpArgs,...(emulateFunctionPointerCasts?["-s","EMULATE_FUNCTION_POINTER_CASTS=1"]:[]),...ltoArgs,"-c","-o",object]);return object});
    const linkedSources=usesChucKRuntime?[...cppSources,adapter]:[adapter,...cppSources];
    execCompiler("em++",[...linkedSources,...objects,...prioritizedIncludeArgs,cppStandard,...optimizationFlags,"-DNDEBUG","-DTEST",...compileDefinitions,...platformDefinitions,...(requiresSseCompat?[requiresSse3?"-msse3":"-msse2","-msimd128"]:externalPffft?["-msimd128"]:[]),...ltoArgs,...linkerArgs,...longjmpArgs,...(emulateFunctionPointerCasts?["-s","EMULATE_FUNCTION_POINTER_CASTS=1"]:[]),...(specializedEffect?["-s","STACK_SIZE=1048576"]:[]),"-s","STANDALONE_WASM=1",...(airwinSuite?["-s","ALLOW_MEMORY_GROWTH=1","-s","MAXIMUM_MEMORY=268435456"]:["-s","ALLOW_MEMORY_GROWTH=0"]),"-s",`INITIAL_MEMORY=${initialMemory}`,"-s",`EXPORTED_FUNCTIONS=${linkedExportedFunctions}`,"--no-entry","-o",artifact]);
  } finally { fs.rmSync(temporaryObjects,{recursive:true,force:true}) }
  return artifact;
}
function staticMemberArraySizes(source){
  const constants={};
  for(const type of declaredTypeNames(source)){
    const body=plainStructBody(source,type);
    if(body===null)continue;
    const pattern=/\b(?:(?:inline|static|constexpr)\s+)*auto\s+([A-Za-z_]\w*)\s*=\s*std::array(?:\s*<[^;{}]+>)?\s*\{/g;
    for(const match of body.matchAll(pattern)){
      if(braceDepthAt(body,match.index)!==0)continue;
      const open=body.indexOf("{",match.index),close=matchingBrace(body,open);
      if(close<0)continue;
      const elements=splitArguments(body.slice(open+1,close)).filter(Boolean);
      constants[`${type}::${match[1]}.size()`]=elements.length;
      for(const [index,element] of elements.entries()){
        const label=/"((?:\\.|[^"\\])*)"/.exec(element);
        if(label)try{constants[`${type}::${match[1]}[${index}].name`]=JSON.parse(`"${label[1]}"`)}catch{}
      }
    }
  }
  return constants
}
function numericConstants(source,initial={}){
  const constants={PORT_MAX_CHANNELS:16,...initial,...(/\bn_osc_params\b/.test(source)?{n_osc_params:7}:{})},
    pattern=/\b(?:(?:static\s+)?auto\s+constexpr|constexpr\s+static|static\s+constexpr|static\s+const|const\s+static|constexpr|const)\s+(?:(?:(?:std::)?size_t|int|unsigned(?:\s+int)?)\s+)?([A-Za-z_]\w*)\s*(?:=\s*([^;]+)|\{\s*([^{};]+)\s*\})\s*;/g,
    multiplePattern=/\b(?:(?:constexpr\s+static|static\s+constexpr|static\s+const|const\s+static|constexpr|const)\s+)(?:(?:std::)?size_t|int|long|unsigned(?:\s+(?:int|long))?)\s+([^;]+)\s*;/g,
    globalPattern=/\b(?:int|long|unsigned(?:\s+(?:int|long))?|(?:std::)?size_t)\s+([A-Z][A-Z0-9_]*)\s*=\s*([^;{}]+)\s*;/g,
    macros=objectMacroDefinitions(source),
    enumerations=[...source.matchAll(/\benum(?:\s+class|\s+struct)?\s+([A-Za-z_]\w*)\b[^;{]*\{([\s\S]*?)\}\s*;/g)].map(match=>parsedEnumInfo(match[1],match[2])),
    anonymousEnumerations=[...source.matchAll(/\benum\s*(?::\s*[A-Za-z_]\w*(?:::[A-Za-z_]\w*)*)?\s*\{([\s\S]*?)\}\s*;/g)].map(match=>parsedEnumInfo("",match[1])),
    enumValues=(info,record)=>{
      let value=0;
      for(const identifier of info.identifiers??[]){
        if(typeof identifier==="string"){
          const assigned=info.assignments?.[identifier];
          if(assigned!==undefined)value=numberLiteral(assigned,value,constants);
          if(Number.isSafeInteger(value)&&value>=0)record(identifier,value);
          value++;
        }else if(identifier&&typeof identifier==="object"){
          if(Number.isSafeInteger(value)&&value>=0)record(identifier.base,value);
          value+=Math.max(0,numberLiteral(identifier.count,0,constants));
        }
      }
    };
  for(let pass=0;pass<4;pass++){
    for(const [name,expression] of macros){const value=numberLiteral(expression,Number.NaN,constants);if(Number.isSafeInteger(value)&&value>=0)constants[name]=value}
    for(const info of enumerations)enumValues(info,(name,value)=>{constants[name]=value});
    const anonymousValues=new Map;
    for(const info of anonymousEnumerations)enumValues(info,(name,value)=>{if(!anonymousValues.has(name))anonymousValues.set(name,new Set);anonymousValues.get(name).add(value)});
    for(const [name,values] of anonymousValues)if(values.size===1&&!Object.hasOwn(constants,name))constants[name]=[...values][0];
    for(const match of source.matchAll(pattern)){const value=numberLiteral(match[2]??match[3],Number.NaN,constants);if(Number.isSafeInteger(value)&&value>=0)constants[match[1]]=value}
    for(const declaration of source.matchAll(multiplePattern))for(const item of splitArguments(declaration[1])){const match=/^\s*([A-Za-z_]\w*)\s*(?:=\s*([\s\S]+)|\{\s*([^{}]+)\s*\})\s*$/.exec(item);if(!match)continue;const value=numberLiteral(match[2]??match[3],Number.NaN,constants);if(Number.isSafeInteger(value)&&value>=0)constants[match[1]]=value}
    for(const match of source.matchAll(globalPattern)){if(!isNamespaceScopeAt(source,match.index))continue;const value=numberLiteral(match[2],Number.NaN,constants);if(Number.isSafeInteger(value)&&value>=0)constants[match[1]]=value}
  }
  for(const match of source.matchAll(/\b(?:(?:static|constexpr)\s+)*(?:(?:std::)?string|(?:const\s+)?char\s*\*)\s+([A-Za-z_]\w*)\s*\[[^\]]*\]\s*=\s*\{([^{};]+)\}\s*;/g))for(const [index,item] of splitArguments(match[2]).entries()){const literal=item.trim();if(/^"(?:\\.|[^"\\])*"$/.test(literal))constants[`${match[1]}_${index}`]=JSON.parse(literal)}
  return constants
}
...
function surgeFxConfigSpecializations(sourceFiles,moduleClass){const type=/\bFX\s*<\s*([^>]+)\s*>/.exec(moduleClass)?.[1]?.trim();if(!type)return"";const escaped=type.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),found=[];for(const file of sourceFiles){const source=fs.readFileSync(file,"utf8"),pattern=new RegExp(`template\\s*<>[\\s\\S]{0,240}?\\bFXConfig\\s*<\\s*${escaped}\\s*>\\s*::\\s*([A-Za-z_]\\w*)\\s*\\(`,"g");for(const match of source.matchAll(pattern)){const prefix=source.slice(0,match.index),insideBlockComment=prefix.lastIndexOf("/*")>prefix.lastIndexOf("*/"),linePrefix=source.slice(prefix.lastIndexOf("\n")+1,match.index);if(insideBlockComment||/\/\//.test(linePrefix)||["getLayout","addFXSpecificMenuItems"].includes(match[1]))continue;const start=match.index,open=source.indexOf("{",match.index),semicolon=source.indexOf(";",match.index);if(open<0||(semicolon>=0&&semicolon<open))continue;const close=matchingBrace(source,open);if(close<0)continue;const definition=source.slice(start,close+1).trim();if(!rackUiPattern.test(definition))found.push(definition)}}return[...new Set(found)].join("\n\n")}
...
function includedClassSource(file,body){
  const root=path.dirname(file),sources=[];
  for(const match of String(body??"").matchAll(/^\s*#include\s+"([^"]+)"\s*$/gm)){const target=path.resolve(root,match[1]);if(target.startsWith(`${root}${path.sep}`)&&fs.existsSync(target))sources.push(fs.readFileSync(target,"utf8"))}
  return sources.join("\n");
}
function directlyIncludedImplementationPrelude(file,sourceDir,alreadyDeclaredNames=new Set){
  ...
  const visit=importer=>{const source=fs.readFileSync(importer,"utf8");for(const match of source.matchAll(/^\s*#include\s+"([^"]+\.(?:cpp|cc|cxx))"\s*$/gm)){const target=resolve(importer,match[1]);if(!target||seen.has(target))continue;seen.add(target);visit(target);const raw=fs.readFileSync(target,"utf8"),systemIncludes=[...raw.matchAll(/^\s*#include\s+<([^>]+)>/gm)].map(candidate=>candidate[1]).filter(browserSafeSystemInclude).map(candidate=>`#include <${candidate}>`),implementationDefines=[...raw.matchAll(/^[ \t]*#[ \t]*define[ \t]+[A-Za-z_]\w*_IMPLEMENTATION(?:[ \t]+[^\r\n]*)?[ \t]*$/gm)].map(candidate=>candidate[0].trim()),localIncludes=[...raw.matchAll(/^\s*#include\s+"([^"]+)"\s*$/gm)].flatMap(candidate=>{const token=candidate[1],header=resolve(target,token);if(!header||!/\.(?:h|hh|hpp|inl)$/.test(header)||["plugin.hpp","rack.hpp"].includes(path.basename(header))||uiHeaderRequired(header,root))return[];const names=declaredDependencyNames(fs.readFileSync(header,"utf8"));return names.some(name=>alreadyDeclaredNames.has(name))?[]:[`#include "${token}"`]}),body=implementationDefines.reduce((value,definition)=>value.replace(definition,""),stripRackUiBlocks(sourceWithoutIncludes(raw)));parts.push([...new Set([...systemIncludes,...implementationDefines,...localIncludes]),body].filter(Boolean).join("\n"))}};
  visit(file);
...
function rackWidgetPlacements(source,enums,constants={},supportSource="",depth=0){
  ...
  for(const match of coordinateText.matchAll(/^\s*#define\s+([A-Za-z_]\w*)\s+((?:mm2px\s*\(\s*)?(?:rack::)?(?:math::)?Vec\s*\([^\n]+\)\s*\)?)\s*$/gm)){const position=rackWidgetPosition(match[2],widgetConstants);if(position&&!vectors.has(match[1]))vectors.set(match[1],position)}
...
function runtimeDraft(target,manifest,moduleManifest,license,detected,moduleClass="",sourceDir=""){
  const constants={...(detected.constants??{}),...(detected.template?.constants??{})},paramIds=enumIds(detected.enums.params,constants),inputIds=enumIds(detected.enums.inputs,constants),outputIds=enumIds(detected.enums.outputs,constants),paramCount=detected.counts?.params??enumCount(detected.enums.params,constants),inputCount=detected.counts?.inputs??enumCount(detected.enums.inputs,constants),outputCount=detected.counts?.outputs??enumCount(detected.enums.outputs,constants),lightCount=detected.counts?.lights??enumCount(detected.enums.lights,constants),params=[];
  for(const entry of [...detected.config.params.map(call=>({call})),...detected.config.switches.map(call=>({call,snap:true})),...detected.config.buttons.map(call=>({call,snap:true,button:true}))]){const values=splitArguments(entry.call),id=enumExpressionId(values[0],paramIds,constants),base=/[A-Za-z_]\w*/.exec(values[0])?.[0],button=entry.button||detected.triggerParamBases?.includes(base),fallback=humanizeEnumIdentifier(String(values[0]??"").split("::").at(-1).replace(/_PARAM$/,""),"PARAM");if(id===undefined||id<0||id>=paramCount)continue;params.push(button?{id,name:stringLiteral(values[entry.button?1:4],fallback,constants),min:0,max:1,default:0,snap:true,button:true}:{id,name:stringLiteral(values[4],fallback,constants),min:numberLiteral(values[1],0,constants),max:numberLiteral(values[2],1,constants),default:numberLiteral(values[3],0,constants),...(entry.snap?{snap:true}:{})})}
...
async function main(){const options=args(process.argv.slice(2)),target=libraryUrl(options.url),fixtureManifest=Boolean(options["manifest-file"]),revisionCache=options["source-cache-dir"]?path.join(path.resolve(options["source-cache-dir"]),"library-index"):path.join(fs.mkdtempSync(path.join(os.tmpdir(),"rack-web-library-")),"index"),cached=cachedSourceCheckout(options["source-cache-dir"],target);let manifest;if(fixtureManifest)manifest=JSON.parse(fs.readFileSync(path.resolve(options["manifest-file"]),"utf8"));else try{manifest=await officialManifest(target.plugin,revisionCache)}catch(error){if(!cached)throw error;manifest=cached.manifest}if(manifest.slug!==target.plugin||!Array.isArray(manifest.modules))fail("Official manifest does not match the requested plugin");const moduleManifest=manifest.modules.find(module=>module.slug===target.model);if(!moduleManifest)fail(`${target.key} is not present in the official plugin manifest`);if(!manifest.sourceUrl)fail(`${target.plugin} does not publish sourceUrl metadata`);const license=String(manifest.license||"");if(!license||license==="proprietary"||/^https?:\/\//.test(license))fail(`${target.plugin} is not declared under an open-source SPDX license`);let sourceCommit,sourceRepositoryUrl=sourceRepository(manifest.sourceUrl);if(!fixtureManifest){const sameCachedSource=cached&&cached.manifest.version===manifest.version&&sourceRepository(cached.manifest.sourceUrl)===sourceRepository(manifest.sourceUrl);if(sameCachedSource)sourceCommit=cached.commit;else{const lock=officialSourceLock(target.plugin,manifest.sourceUrl,manifest.version,revisionCache);sourceCommit=lock.commit;sourceRepositoryUrl=lock.repository}}
  let sourceDir=options["source-dir"]&&path.resolve(options["source-dir"]),temporary=false;if(!sourceDir){if(options["source-cache-dir"]){sourceDir=path.join(path.resolve(options["source-cache-dir"]),target.plugin,sourceCommit);if(!fs.existsSync(path.join(sourceDir,"plugin.json")))checkoutSource(sourceRepositoryUrl,sourceCommit,sourceDir)}else{sourceDir=path.join(fs.mkdtempSync(path.join(os.tmpdir(),"rack-web-source-")),"checkout");temporary=true;checkoutSource(sourceRepositoryUrl,sourceCommit,sourceDir)}}if(!fs.existsSync(path.join(sourceDir,"plugin.json")))fail("Checked-out source does not contain plugin.json");if(!fixtureManifest&&options["source-dir"]){const explicitCommit=gitValue(sourceDir,["rev-parse","HEAD"]);if(!/^[0-9a-f]{40}$/.test(explicitCommit))fail("Explicit source directory must be an exact Git checkout");sourceCommit=explicitCommit}const selectedSubmodules=target.key==="ParableInstruments/Neil"?new Set(["parasites/stmlib"]):null;initializeSubmodules(sourceDir,0,selectedSubmodules);const sourceManifest=JSON.parse(fs.readFileSync(path.join(sourceDir,"plugin.json"),"utf8"));if(sourceManifest.slug!==manifest.slug)fail("Source plugin.json slug differs from the official manifest");if(!fixtureManifest&&sourceManifest.version!==manifest.version)fail(`Source version ${sourceManifest.version} differs from Library version ${manifest.version}`);hydrateLockedFallbackDependencies(sourceDir);const browserLuaRuntime=hydrateWrongPeopleLuaRuntime(sourceDir,target);
  let rootSourceFiles=target.key==="ParableInstruments/Neil"?files(path.join(sourceDir,"src")):filesOutsideNestedRepositories(sourceDir),
    registrationConstants=registrationStringConstants(rootSourceFiles),
    registrations=[];
  for(const file of rootSourceFiles){const source=fs.readFileSync(file,"utf8");registrations.push(...macroConfiguredRegistrations(source,file,registrationConstants))}
  if(!registrations.some(item=>item.slug===target.model)){
    ...
    if(registrations.some(item=>item.slug===target.model))rootSourceFiles=nestedSourceFiles;
  }
  ...
  const inlineHeaderFunctions=localSourceFiles
    .filter(file=>/\.(?:h|hh|hpp)$/.test(file))
  ...
  if(process.env.RACK_WEB_DEBUG_DEPENDENCIES&&inlineHeaderFunctions.length)console.error(JSON.stringify({inlineHeaderFunctions:inlineHeaderFunctions.map(value=>freeFunctionDeclaration(value))},null,2));
  ...
  if(process.env.RACK_WEB_DEBUG_CONSTANTS)console.error(JSON.stringify({memberArraySizes,constantOverrides,configuredCounts,inherited:inherited.map(item=>({name:item.name,actualType:item.actualType,analysisBody:item.analysisBody}))},null,2));
  ...
  const targetSourceFreeFunctions=referencedLocalFreeFunctionDefinitions(definition.source,[body,...inheritedSource].join("\n")).map(value=>adaptEmbeddedBinaryAssetLoads(adaptEmbeddedMidiAssetLoads(adaptEmbeddedJsonAssetLoads(value,embeddedJsonAssets),embeddedMidiAssets),embeddedBinaryAssets)),targetSourceFreeFunctionGlobals=namespaceGlobalDefinitions(definition.source,targetSourceFreeFunctions.join("\n"));
  ...
  for(const helper of paramQuantityHelpers(rawModulePreludeSource,body)){const preferredImplementations=outOfLineDefinitions(fs.readFileSync(definition.file,"utf8"),helper.name,true).filter(value=>!rackUiPattern.test(value)),implementationFiles=preferredImplementations.length?[definition.file]:activeRootSourceFiles;for(const file of implementationFiles)for(const implementation of(file===definition.file?preferredImplementations:outOfLineDefinitions(fs.readFileSync(file,"utf8"),helper.name,true).filter(value=>!rackUiPattern.test(value))))if(!implementations.includes(implementation))implementations.push(implementation)}
  ...
  for(const base of secondaryBases)for(const file of rootSourceFiles)for(const definition of outOfLineDefinitions(fs.readFileSync(file,"utf8"),base).filter(value=>!rackUiPattern.test(value)))if(!implementations.includes(definition))implementations.push(definition);
  ...
  const onOffCalls=[...calls(configuredSource,"configOnOff",constants),...calls(configuredSource,"configOnOffNoRand",constants)].map(call=>{const values=splitArguments(call);return`${values[0]}, 0, 1, ${values[1]}, ${values[2]}`});
  ...
  for(const [kind,names] of Object.entries({params:["ParamIds","ParamsIds","ParamId"],inputs:["InputIds","InputsIds","InputId"],outputs:["OutputIds","OutputsIds","OutputId"],lights:["LightIds","LightsIds","LightId"]})){
    ...
    const targetGlobalEnum=enumRecordInNamespace(definition.source,names,definition.namespace??[]);
    if(targetGlobalEnum){
...
${dependencyBundle.source??""}`;
  ...
  if(target.model.startsWith("SurgeXTFX")){
    ...
    detected.config.inputs.push(...calls(fxPortConfigSource,"configInput",constants));
    detected.config.outputs.push(...calls(fxPortConfigSource,"configOutput",constants));
  }
  ...
  if(configuredIds.length===3&&!detected.enums.lights)detected.counts.lights=0;
...
${prelude}`:prelude;
  ...
  effectivePrelude=effectivePrelude.replace(/^\s*#include\s+["<]([^">]+)[">]\s*$/gm,(line,include)=>{
    const normalized=include.split(/[\\/]+/).join(path.sep);
    return inlinedLocalDependencyFiles.some(file=>file.endsWith(`${path.sep}${normalized}`))?"":line;
  });
